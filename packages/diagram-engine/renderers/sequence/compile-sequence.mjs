...
validateSequence();
return { svg: renderSvg(), viewBox, objects: [...participants.values()].map((node) => ({id:node.id, label:node.label, x:node.x, y:node.y, width:node.width, height:node.height})), groups: [], relationships: asArray(sequence.messages).map((edge) => ({...edge, ...(() => { const {start, end} = messageGeometry(edge); return { points: [[start, edge.y], [end, edge.y]], d: `M ${start} ${edge.y} L ${end} ${edge.y}` }; })()})) };
}
