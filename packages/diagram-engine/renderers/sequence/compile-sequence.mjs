...
export function compileSequence(sequence) {
  ...
  const participantW =
    columnFit === "spread"
      ? Math.max(
          86,
          Math.min(190, Math.round((viewBox[0] - sideMargin * 2) / participantCount) - 24),
        )
      : 86;
  const colGap =
...
${renderLegend()}
  ...
  return {
    svg: renderSvg(),
    viewBox,
    objects: [...participants.values()].map((node) => ({
      id: node.id,
      label: node.label,
      x: node.x,
      y: node.y,
    ...
    groups: [],
    relationships: asArray(sequence.messages).map((edge) => ({
      ...edge,
      ...(() => {
        const { start, end } = messageGeometry(edge);
        return {
          points: [
            [start, edge.y],
            [end, edge.y],
          ],
          d: `M ${start} ${edge.y} L ${end} ${edge.y}`,
        };
      })(),
    })),
  };
