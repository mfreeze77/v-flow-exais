...
export function compileLifecycle(lifecycle) {
...
validateLifecycle();
return { svg: renderSvg(), viewBox, objects: [...states.values()].map((node) => ({id:node.id, label:node.label, x:node.x, y:node.y, width:node.width, height:node.height})), groups: [], relationships: asArray(lifecycle.transitions).map((edge) => ({...edge, ...pathFor(edge)})) };
}
