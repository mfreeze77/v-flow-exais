...
export function compileLifecycle(lifecycle) {
  ...
  function renderState(state) {
    ...
    const passport = {
      ...
      tag: state.tag,
      context:
        laneLabels.get(state.lane) || i18nText(lifecycle.meta.locale, "node.context.lifecycle"),
...
${renderLegend()}
  ...
  return {
    svg: renderSvg(),
    viewBox,
    objects: [...states.values()].map((node) => ({
      id: node.id,
      label: node.label,
      x: node.x,
      y: node.y,
    ...
    groups: [],
    relationships: asArray(lifecycle.transitions).map((edge) => ({ ...edge, ...pathFor(edge) })),
  };
}
