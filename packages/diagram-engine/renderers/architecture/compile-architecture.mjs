...
return { svg: renderSvg(), report: buildLayoutReport(), viewBox, objects: [...components.values()].map(componentBox), groups: boundaries.map(boundaryBox), relationships: asArray(arch.connections).map((edge) => ({ ...edge, ...pathFor(edge), labelAt: edge.label ? labelPoint(edge, pathFor(edge).points) : null })) };
}
