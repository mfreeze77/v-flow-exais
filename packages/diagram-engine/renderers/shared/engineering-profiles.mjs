...
export function deploymentOwnershipDiagnostics(diagram) {
  ...
  const boundaries = (Array.isArray(diagram.boundaries) ? diagram.boundaries : []).map(
    (boundary) => ({ ...boundary, wraps: Array.isArray(boundary.wraps) ? boundary.wraps : [] }),
  );
  ...
  boundaries.forEach((boundary, index) => {
    ...
    const regionIndexes = new Set(
      members.flatMap((member) => member.regions.map((region) => region.boundaryIndex)),
    );
    const consistent =
      members.length > 0 &&
  ...
  connections.forEach((connection, index) => {
    const crossedBoundaries = boundaries
      ...
      .filter(
        (boundary) =>
          DEPLOYMENT_BOUNDARY_KINDS.has(boundary.kind) && boundary.fromInside !== boundary.toInside,
      );
    if (
    ...
    diagnostics.push({
      ...
      evidence: {
        ...
        to: connection.to,
        crossedBoundaries: crossedBoundaries.map(({ boundaryIndex, kind, label }) => ({
          boundaryIndex,
          kind,
          label,
        })),
