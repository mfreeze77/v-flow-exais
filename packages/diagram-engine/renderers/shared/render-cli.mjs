...
export async function runRenderer(kind, defaultExample) {
  ...
  const loaded = await loadDiagramWithBrandMarks({ diagramType: kind, defaultExample, argv: process.argv.filter((arg) => arg !== '--layout-json') });
  const marks = Object.values(loaded.diagram).filter(Array.isArray).flat().map(brandMarkFor).filter(Boolean);
  const result = await compileDiagram({ kind, source: loaded.diagram, options: { qualityProfile: process.env.ARCHIFY_QUALITY_PROFILE, standaloneAnimation: true },
  ...
  if (!result.ok) throwDiagnosticError(result.diagnostics.map((diagnostic) => diagnostic.message).join('\n'), result.diagnostics);
  if (layoutJson) {
    process.stdout.write(`${JSON.stringify(result.artifact.report, null, 2)}\n`);
    return;
  }
  writeDiagram({ ...loaded, diagramType: kind, meta: loaded.diagram.meta, cards: loaded.diagram.cards, svg: result.artifact.svg });
}
