...
export function loadDiagram({ diagramType, defaultExample, argv = process.argv }) {
  // AFM-011: assets resolve from package-owned paths, not from an assumed skill
  // root two levels above the renderer. The viewer template lives in
  // @hyperframes/diagram-viewer, so resolving relative to the renderer's own
  // directory no longer finds it — which is why this no longer takes one.
