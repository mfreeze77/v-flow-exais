...
export function compileDataflow(dataflow) {
...
validateDataflow();
return { svg: renderSvg(), viewBox, objects: [...nodes.values()].map((node) => ({id:node.id, label:node.label, x:node.x, y:node.y, width:node.width, height:node.height})), groups: [], relationships: asArray(dataflow.flows).map((edge) => ({...edge, ...pathFor(edge)})) };
}
