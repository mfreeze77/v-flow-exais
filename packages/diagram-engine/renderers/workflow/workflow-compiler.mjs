...
function readableGroupBounds(workflow, group, colXs) {
  ...
  let left =
    group.fromCol === group.toCol && width > naturalWidth ? start : (start + end - width) / 2;
...
function verticalIntervalsOverlap(a, b, clearance = 0) {
  ...
  return (
    Math.abs(aCenter - bCenter) < authoredNodeHeight(a) / 2 + authoredNodeHeight(b) / 2 + clearance
  );
}
...
function createReadableLayout(workflow, layoutFeedback = {}) {
  ...
  for (const edge of asArray(workflow.edges)) {
    ...
    if (!fromNode || !toNode || fromNode.lane !== toNode.lane || fromNode.col === toNode.col)
      continue;
    if (
      edge.via ||
    ...
    const labeledDirectClearance =
      edge.label && !edge.labelAt ? Math.max(28, workflowLabelWidth(edge.label) + 8) : 28;
    ...
    const preferLabelChannel =
      canUseAutomaticLabelChannel && directLabelExpansionCost > channelDetourBudgetPx;
    ...
    if (!preferLabelChannel && labeledDirectClearance > 28) {
      const labelConstraintMinimum =
        authoredNodeWidth(earlier) / 2 + labeledDirectClearance + authoredNodeWidth(later) / 2;
  ...
  for (const nodeId of unpinnedTopEndpointIds) {
    ...
    if (!lane) continue;
    const prefix = lane.variant === "exception" ? "EX" : String(lanePosition + 1).padStart(2, "0");
  ...
  for (const node of nodes) {
    ...
    const nodeContributors = new Set([...colProvenance[node.col], nodeWidthContributor(node)]);
  ...
  for (const group of asArray(workflow.groups)) {
    ...
    const groupContributors = new Set([
      ...
      ...nodes
        .filter(
          (node) =>
            node.lane === group.lane && node.col >= group.fromCol && node.col <= group.toCol,
        )
  ...
  const groupLaneReserves = asArray(workflow.lanes).map((lane) => {
    ...
    for (const group of groupsByLane.get(lane.id) || []) {
      ...
      for (const node of nodes) {
        ...
        const topOffset =
          (baseContentH - authoredNodeHeight(node)) / 2 + (Number(node.yOffset) || 0);
        const minimumTopOffset = overlapsLabel ? 11 : 9;
        header = Math.max(header, Math.ceil(minimumTopOffset - topOffset));
        const bottomMargin =
          baseContentH - GROUP_FRAME_BOTTOM_INSET - topOffset - authoredNodeHeight(node);
...
function compileWorkflowInternal({
  ...
  const qualityResolvedWorkflow =
    resolvedQualityProfile && resolvedQualityProfile !== inputWorkflow.meta?.quality_profile
      ? {
          ...inputWorkflow,
          meta: { ...inputWorkflow.meta, quality_profile: resolvedQualityProfile },
        }
      : inputWorkflow;
  ...
  const legendExtraHeight = workflow.schema_version === 2 ? packedLegendFootprint.extraHeight : 0;
  ...
  function measureNode(node) {
    ...
    const contentH =
      laneHeight(node.lane) - layout.laneTitleH - groupHeaderH - laneGroupFooterH(node.lane);
    const y =
      laneTop(node.lane) +
      layout.laneTitleH +
      groupHeaderH +
  ...
  function verifiedReducedWidths(from, to, requiredClearance) {
    ...
    const signedClearance =
      Math.abs(to.cx - from.cx) - serializedWidths[0] / 2 - serializedWidths[1] / 2;
  ...
  function enforceLegacyColumnCapacity() {
    ...
    for (const edge of workflow.edges) {
      ...
      if (alternative !== null)
        supportedFixes.push(`move node "${to.id}" to verified free column ${alternative}`);
      if (reducedWidths) {
        supportedFixes.push(
    ...
    for (let size = 1; size <= uniqueFields.length; size += 1) {
      ...
      return {
        ...
        supportedFixes: removalSets.map(
          (fieldSet) => `remove ${fieldSet.join(" and ")} from edge "${edgeName}" ${reason}`,
        ),
      };
    }
  ...
  function hasAbsoluteRoutePins(edge) {
    return Array.isArray(edge?.via) || edge?.channelX !== undefined || edge?.channelY !== undefined;
  }
  ...
  function throwReadableLabelRoutePinConflict(hit, routePoints = null) {
    ...
    const actualRoutePoints =
      routePoints || pathCache.get(routeEdge)?.points || pathFor(routeEdge).points;
    const diagnosticEdge =
      alternatives.conflictingRefs[0]?.edge || (labelPinned ? labelEdge : routeEdge);
  ...
  function classifyFailedAutomaticCandidatePins(edge, rawCandidates) {
    ...
    const priorLabels = priorRoutes
      .map(({ relation, relationIndex }) => labelRectFor(relation, relationIndex))
      .filter(Boolean);
    ...
    for (const { points } of rawCandidates) {
      ...
      if (candidateLabel) {
        ...
        const labelRouteHit = collectLabelRouteClearance({
          ...
          threshold: 4,
        }).find((hit) => Array.isArray(edge.labelAt) || hasAbsoluteRoutePins(hit.otherRelation));
        if (labelRouteHit) {
  ...
  function corridorTopologyMatches(points, axis, coordinate) {
    ...
    return (
      ...
      routeContainsChannelPin(collapsed, axis === "x" ? "channelX" : "channelY", coordinate)
    );
  }
  ...
  function routeContainsChannelPin(points, field, value) {
    return points.slice(0, -1).some((start, index) => {
      ...
      if (field === "channelX") {
        return start[0] === value && end[0] === value && Math.abs(end[1] - start[1]) > 0.0001;
      }
      return start[1] === value && end[1] === value && Math.abs(end[0] - start[0]) > 0.0001;
    });
  }
    ...
    for (const edge of workflow.edges) {
      ...
      if (negativeRoutePin) {
        throwExplicitPinConflict(
          ...
          [verifiedAutomaticRouteFix(edge), verifiedAutomaticRouteFix(edge, { clearSides: true })],
        );
      }
      const hasPinnedRoute =
        Array.isArray(edge.via) || edge.channelX !== undefined || edge.channelY !== undefined;
      ...
      if (hasPinnedRoute) {
        ...
        for (let segmentIndex = 0; segmentIndex < points.length - 1; segmentIndex += 1) {
            ...
            const duplicateFix =
              Array.isArray(edge.via) && edge.via.length
                ? verifiedEdgeFix(
                    ...
                    (candidate) =>
                      candidate.via.splice(Math.min(segmentIndex, candidate.via.length - 1), 1),
                  )
          ...
          if (lengthPx + 0.0001 < minimumPx) {
            ...
            throwExplicitPinConflict(
              edge,
              endpoint ? "8px endpoint stub clearance" : "16px interior turn clearance",
              {
                conflictingPins: alternatives.conflictingPins,
                segmentIndex,
                position:
        ...
        if (
          edge.route &&
          edge.route !== "auto" &&
          !routeMatchesPresetFamily(edge.route, points, nodes.get(edge.from), nodes.get(edge.to))
        ) {
        ...
        if (!routeHonorsEndpointSides(points, fromSide, toSide)) {
          ...
          const candidateFields = [...mismatchedSideFields, ...presentRouteGeometryFields(edge)];
          const alternatives = verifiedPinRemovalAlternatives(
            edge,
            candidateFields,
      ...
      if (edge.labelAt) {
        ...
        const legendObstacle =
          rect && workflowLegendRects().find((legendRect) => rectsOverlap(rect, legendRect));
        if (legendObstacle) {
          throwExplicitPinConflict(
            edge,
        ...
        const compositionObstacle =
          rect && workflowSceneLabelObstacles().find((candidate) => rectsOverlap(rect, candidate));
        if (compositionObstacle) {
          throwExplicitPinConflict(
            edge,
  ...
  function validateReadableInputsBeforeRouting() {
    ...
    const authoredLanes = [...workflow.lanes].sort(
      (left, right) => sourceIndexes.lanes.get(left) - sourceIndexes.lanes.get(right),
    );
    const authoredNodes = [...workflow.nodes].sort(
      (left, right) => sourceIndexes.nodes.get(left) - sourceIndexes.nodes.get(right),
    );
    const authoredEdges = [...workflow.edges].sort(
      (left, right) => sourceIndexes.edges.get(left) - sourceIndexes.edges.get(right),
    );
  ...
  function routeClearsUnrelatedNodes(edge, points, clearance = 2) {
    ...
    for (const node of nodes.values()) {
      ...
      for (let index = 0; index < points.length - 1; index += 1) {
        if (
          segmentIntersectsRect({ start: points[index], end: points[index + 1] }, node, clearance)
        ) {
          return false;
        }
      }
  ...
  function oneBendCrossLaneVia(edge, start, end, fromSide, toSide) {
    ...
    const segmentsAreReadable = points
      ...
      .every(
        (point, index) =>
          Math.hypot(points[index + 1][0] - point[0], points[index + 1][1] - point[1]) >= 8,
      );
  ...
  function routeClearsPlacedLabels(edge, points) {
    ...
    for (const [otherEdge, routed] of pathCache) {
      const otherIndex = workflow.edges.indexOf(otherEdge);
      const otherLabel = labelRectFor(otherEdge, otherIndex);
      if (candidateLabel && otherLabel && rectsOverlap(candidateLabel, otherLabel, -2))
        return false;
      if (candidateLabel) {
        for (let index = 0; index < routed.points.length - 1; index += 1) {
          const clearance = segmentRectClearance(
  ...
  function routeClearsLegend(edge, points) {
    ...
    for (const rect of legendRects) {
      for (let index = 0; index < points.length - 1; index += 1) {
        if (segmentIntersectsRect({ start: points[index], end: points[index + 1] }, rect))
          return false;
      }
  ...
  function automaticForwardReversePx(edge, points) {
    ...
    return points
      .slice(0, -1)
      .reduce((total, point, index) => total + Math.max(0, point[0] - points[index + 1][0]), 0);
  }

  function readableCandidateCost(edge, points, ordinal, naturalFromSide, naturalToSide) {
  ...
  function readableAutomaticCandidateSet(
    ...
    fromSide,
    toSide,
    { ordinalOffset = 0, naturalFromSide = fromSide, naturalToSide = toSide } = {},
  ) {
    const midX = (start[0] + end[0]) / 2;
    const laneGapY =
      from.lane === to.lane
    ...
    const bottomY =
      Math.max(laneTop(from.lane) + laneHeight(from.lane), laneTop(to.lane) + laneHeight(to.lane)) +
      16;
    ...
    const candidates = rawCandidates
      ...
      .filter(({ points }) => readableCandidateIsFeasible(edge, points, from, to, fromSide, toSide))
      .map((candidate) => ({
        ...candidate,
  ...
  function readableAutomaticVia(edge, from, to, start, end, fromSide, toSide) {
        ...
        if (outsideRightMinX > outsideRight + 0.0001) {
          ...
          if (readableCandidateIsFeasible(edge, expandedPoints, from, to, fromSide, toSide)) {
            ...
            for (
              ...
              if (readableCandidateIsFeasible(edge, midpointPoints, from, to, fromSide, toSide)) {
                feasibleX = midpointX;
    ...
    const hasRelevantAbsolutePin =
      Array.isArray(edge.labelAt) ||
      [...pathCache.keys()].some(
        (otherEdge) => Array.isArray(otherEdge.labelAt) || hasAbsoluteRoutePins(otherEdge),
      );
  ...
  function workflowEdgeLabelPoint(edge, points) {
    ...
    const segments = points
      ...
      .map((point, index) => ({
        ...
        length: Math.hypot(points[index + 1][0] - point[0], points[index + 1][1] - point[1]),
      }))
      .sort(
        (left, right) =>
  ...
  function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
    ...
    for (const candidateSides of sidePairs) {
      ...
      const start =
        primaryPorts?.from && primary ? primaryPorts.from : anchor(from, candidateSides.fromSide);
      const end = primaryPorts?.to && primary ? primaryPorts.to : anchor(to, candidateSides.toSide);
  ...
  function readableControlledRoute(edge, from, to) {
    ...
    for (const [ordinal, { fromSide, toSide }] of sidePairs.entries()) {
      ...
      const points = hasAbsoluteRoutePins ? authoredPoints : normalizeRoutePoints(authoredPoints);
      ...
      candidates.push({
        ...
        fromSide,
        toSide,
        cost: readableCandidateCost(edge, points, ordinal, naturalFromSide, naturalToSide),
      });
    }
    candidates.sort((left, right) => compareCost(left.cost, right.cost));
    ...
    return {
      ...fallback,
      points: normalizeRoutePoints([fallback.points[0], ...fallbackVia, fallback.points.at(-1)]),
    };
  }
  ...
  function pathFor(edge) {
    ...
    if (readableAutomatic) {
      const planned = readableAutomaticRoute(edge, from, to, { fromSide, toSide }, ports);
      readableSideCache.set(edge, {
        fromSide: planned.fromSide,
        toSide: planned.toSide,
      });
