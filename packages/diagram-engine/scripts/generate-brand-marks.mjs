...
function lookupForms(value) {
  ...
  if (!raw) return [];
  return [...new Set([raw, raw.replace(/[\s_]+/g, "-"), raw.replace(/[\s_.-]+/g, "")])];
}
...
const generated = catalog.marks
  .map((entry, index) => {
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry.id || ""))
      fail(`marks[${index}] has an invalid id`);
    ...
    for (const key of [entry.id, ...aliases]) {
      for (const form of lookupForms(key)) {
        if (lookupKeys.has(form) && lookupKeys.get(form) !== entry.id) {
          fail(
            `lookup key ${JSON.stringify(key)} is shared by ${lookupKeys.get(form)} and ${entry.id}`,
          );
        }
    ...
    for (const form of lookupForms(mark.title)) {
      if (lookupKeys.has(form) && lookupKeys.get(form) !== entry.id) {
        fail(
          `title ${JSON.stringify(mark.title)} is shared by ${lookupKeys.get(form)} and ${entry.id}`,
        );
      }
      lookupKeys.set(form, entry.id);
    }
