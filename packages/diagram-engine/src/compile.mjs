...
export async function compileDiagram(request) {
  ...
  return withCompilationContext(context, async () => {
    try {
      ...
      if (!Object.hasOwn(objectKeys, kind))
        return {
          ok: false,
          diagnostics: [fail("diagram/kind", "Choose one of the five supported diagram kinds.")],
        };
      if (!["dark", "light"].includes(theme))
        return { ok: false, diagnostics: [fail("diagram/theme", "Theme must be dark or light.")] };
      if (options.qualityProfile && !["standard", "showcase"].includes(options.qualityProfile))
        return {
          ok: false,
          diagnostics: [fail("diagram/quality", "Unsupported quality profile.")],
        };
      if (preset && !["classic", "signal-flow", "blueprint", "editorial"].includes(preset))
        return { ok: false, diagnostics: [fail("diagram/preset", "Unsupported visual preset.")] };
        ...
        resolveAsset: resolveAsset
          ? async (reference) => {
              ...
              if (!asset || !["image/png", "image/jpeg", "image/webp"].includes(asset.mimeType))
                throw new Error("Resolved brand assets must be local PNG, JPEG or WebP bytes.");
              if (!(asset.bytes instanceof Uint8Array) || asset.bytes.length > 2_000_000)
                throw new Error("Resolved brand asset exceeds its byte limit.");
              const bytes = Buffer.from(asset.bytes);
              return {
                id: reference.sha256,
                title: asset.title || "Pinned brand",
                kind: "remote",
                status: "captured",
      ...
      if (kind === "workflow") {
        const result = compileWorkflow({
          workflow: diagram,
          qualityProfile: options.qualityProfile ?? diagram.meta.quality_profile,
        });
        if (!result.ok)
          return {
            ok: false,
            diagnostics: result.diagnostics || context.diagnostics,
            report: result.receipt,
          };
        compiled = {
          svg: result.svg,
          viewBox: result.receipt.viewBox,
          objects: result.receipt.nodes,
          relationships: result.receipt.edges,
          groups: [],
          report: result.receipt,
        };
      ...
      const objects = compiled.objects.map((node) => ({
        id: node.id,
        kind: "object",
        label: sourceObjects.get(node.id)?.label || node.label || node.id,
      ...
      const relationships = compiled.relationships.map((edge, index) => {
        ...
        if (authored.id == null)
          diagnostics.push({
            code: "identity/migration-required",
            severity: "warning",
            pointer: `/${edgeKeys[kind]}/${index}/id`,
            message:
              "Assign and persist a stable relationship ID during project import before animation.",
          });
        return {
          id: authored.id ?? null,
          kind: "relationship",
          ...
          path: edge.d || "",
          points: edge.points,
          labelAt: edge.labelAt ?? null,
          sourcePointer: `/${edgeKeys[kind]}/${index}`,
        };
      });
      const evidence = resolveEvidence
        ? await resolveEvidence({ kind, source: structuredClone(source), sourceHash })
        : null;
      const artifact = {
        schemaVersion: 1,
        ...
        sourceHash,
        theme,
        preset: preset || source.meta.visual_preset || "classic",
        svg: compiled.svg,
        styles: diagramStyles,
        viewBox: [0, 0, ...compiled.viewBox],
        objects,
        groups: compiled.groups,
        relationships,
        guidedViews: structuredClone(source.meta.views || []),
        diagnostics,
        evidence,
      ...
      const diagnostics = error?.archifyDiagnostics?.length
        ? error.archifyDiagnostics.map((entry) => ({
            ...entry,
            pointer: entry.subject?.instancePath || entry.subject?.path || "/",
          }))
