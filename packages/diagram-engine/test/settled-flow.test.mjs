...
test("settled flow restores authored security and async dash semantics", () => {
  ...
  const runningRule =
    template.match(
      /html\[data-ambient-motion="running"\][^{]+\[data-animate="edge"\][^{]*\{([^}]*)\}/,
    )?.[1] || "";
...
test("ambient ownership is generation-bounded and cannot replay after settle", () => {
  ...
  assert.match(
    template,
    /ambientStarted = true;[\s\S]*?html\.setAttribute\('data-ambient-motion', 'running'\)/,
  );
...
test("animation delay is capped without changing normal authored order", () => {
  assert.equal(
    animateAttr({ animation: "trace" }, "edge", 0),
    ' data-animate="edge" style="--step:0"',
  );
  assert.equal(
    animateAttr({ animation: "trace" }, "node", 8),
    ' data-animate="node" style="--step:8"',
  );
  assert.equal(
    animateAttr({ animation: "trace" }, "edge", 99),
    ' data-animate="edge" style="--step:12"',
  );
