...
test("release prevents manifest preannouncement and smokes the exact archive before upload", () => {
  ...
  const publicationOrder = workflowStep(
    workflow,
    "Stable notifier manifest must remain on the previous release",
  );
  ...
  assert.ok(
    workflow.indexOf(tagFetch) < workflow.indexOf(tagGate),
    "the real tag object must be fetched before release identity checks",
  );
  assert.ok(
    workflow.indexOf(tagGate) < workflow.indexOf(publicationOrder),
    "tag/version gate must precede the publication-order gate",
  );
  assert.ok(
    workflow.indexOf(tagGate) < workflow.indexOf(annotatedTagGate),
    "tag/version gate must precede the annotated-tag gate",
  );
  assert.ok(
    workflow.indexOf(annotatedTagGate) < workflow.indexOf(publicationOrder),
    "annotated-tag gate must precede the publication-order gate",
  );
  assert.ok(
    workflow.indexOf(publicationOrder) < workflow.indexOf(build),
    "manifest preannouncement must fail before the release build",
  );
  assert.ok(
  ...
  assert.ok(
    ...
    "release smoke must inspect the built archive before comparison",
  );
  assert.ok(
  ...
  assert.ok(
    workflow.indexOf(upload) < workflow.indexOf(followUp),
    "manifest follow-up must be recorded only after Release creation",
  );
...
test("an exact tag fetch restores an annotated object after a SHA-only checkout", () => {
  ...
  try {
    ...
    assert.equal(
      runGit(checkout, ["fetch", "--no-tags", "--depth=1", "origin", `+${commit}:refs/tags/v1.0.0`])
        .status,
      0,
    );
...
test("package smoke rejects a missing or modified distribution license", () => {
  ...
  try {
    ...
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /missing the exact Cocoon AI copyright line/,
    );
...
test("package smoke rejects missing, modified, or incomplete third-party notices", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      noticesPath,
      repositoryNotices.replace("Simple Icons 16.28.0", "Simple Icons"),
    );
    result = spawnSync(process.execPath, [packageSmoke, staged], { encoding: "utf8" });
    ...
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /packaged THIRD_PARTY_NOTICES\.md is incomplete/,
    );
  ...
  () => {
    ...
    try {
      ...
      assert.doesNotMatch(
        listing.stdout,
        new RegExp(marker),
        "untracked files and symlinks must not enter the archive",
      );
    } finally {
...
canonicalZipTest(
  "archive build rejects an unmerged index and preserves an existing archive",
  () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(skill, "renderers", "shared", "generated-validators.mjs"),
        "export default {};\n",
      );
      ...
      assert.ok(
        fs.readFileSync(archive).equals(trusted),
        "a failed build must preserve the trusted archive",
      );
    } finally {
      fs.rmSync(fixture, { recursive: true, force: true });
    }
...
test(
  "archive build rejects non-canonical Node versions before publishing output",
  {
    skip:
      currentNodeMajor === canonicalZipNodeMajor
  ...
  () => {
    ...
    try {
      ...
      assert.ok(
        fs.readFileSync(archive).equals(trusted),
        "version rejection must preserve the canonical archive",
      );
    } finally {
      fs.rmSync(outputRoot, { recursive: true, force: true });
    }
...
canonicalZipTest(
  "archive build is byte-for-byte reproducible across caller time zones without system zip",
  () => {
