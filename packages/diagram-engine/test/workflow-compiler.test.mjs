...
function numericRect(attributes) {
  return Object.fromEntries(
    ["x", "y", "width", "height"].map((name) => [name, Number(attribute(attributes, name))]),
  );
}

function nodeRect(svg, id) {
  const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const attributes = svg.match(
    new RegExp(`<g\\b[^>]*id="node-${escapedId}"[^>]*>[\\s\\S]*?<rect\\b([^>]*)>`),
  )?.[1];
...
function edgePoints(svg, id) {
  const tag = (svg.match(/<path\b[^>]*>/g) || []).find(
    (candidate) => attributeOrUndefined(candidate, "data-edge-id") === id,
  );
...
function compileSuccessfully(workflow, qualityProfile) {
  const request = qualityProfile === undefined ? { workflow } : { workflow, qualityProfile };
...
function assertReadableAdjacentResult(result, { label, widths = [92, 92] } = {}) {
  ...
  assert.deepEqual(
    points.length,
    2,
    `adjacent facing nodes should use a direct route: ${JSON.stringify(points)}`,
  );
...
test("fixed-v1 compiler preserves the official workflow baseline SVG byte-for-byte", () => {
  const workflow = readJson(
    path.join(__dirname, "fixtures", "v1-baseline", "agent-tool-call.workflow.json"),
  );
...
test("fixed-v1 compiler preserves the exact 700x400 compatibility geometry", () => {
  const workflow = readJson(path.join(__dirname, "fixtures", "v1-workflow-700x400.workflow.json"));
      ...
      for (const label of [undefined, "liga"]) {
        for (const frames of [false, true]) {
          for (const width of [900, 1080, 1400, 1600]) {
            const result = compileSuccessfully(
              adjacentWorkflow({
                fromCol,
                label,
                frames,
...
test("readable-v2 phase and group frames derive from solved ranks without moving the core geometry", () => {
  for (const qualityProfile of ["standard", "showcase"]) {
    for (let fromCol = 0; fromCol < 5; fromCol += 1) {
      const plain = compileSuccessfully(
        adjacentWorkflow({ fromCol, label: "liga" }),
        qualityProfile,
      );
...
test("readable-v2 never routes through a single-rank group label", () => {
  ...
  const attributes = rendered.svg.match(new RegExp(`<text\\b([^>]*)>${escapedLabel}</text>`))?.[1];
...
test("readable-v2 shifts top-side routes beyond lane header text deterministically", () => {
  ...
  assert.ok(
    compiled.get("Responsibility owner").receipt.columns[0] > compiled.get("R").receipt.columns[0],
...
test("explicit viewBox width is containment capacity and never stretches readable-v2 geometry", () => {
  for (const qualityProfile of ["standard", "showcase"]) {
    for (let fromCol = 0; fromCol < 5; fromCol += 1) {
      const intrinsic = compileSuccessfully(
        adjacentWorkflow({
          fromCol,
          label: "liga",
        }),
...
test("capacity-only viewBox failures preserve the intrinsic routes, labels, and requirement", () => {
  ...
  assert.equal(
    insufficient.diagnostics.length,
    1,
    JSON.stringify(insufficient.diagnostics, null, 2),
  );
...
test("explicit viewBox capacity names the rank and authored node widths that require more width", () => {
  ...
  assert.ok(
    contributors.some(
      (contributor) => /rank 1→2/i.test(contributor) && /clearance/i.test(contributor),
    ),
...
test("readable-v2 compares long-label gutter growth with a legal automatic channel", () => {
  ...
  assert.ok(
    edgePoints(automatic.svg, "ab").length > 2,
    "the compact plan must use a legal channel",
  );
  assert.ok(
...
test("readable-v2 contains a long single-rank group label in its frame and intrinsic canvas", () => {
  ...
  assert.ok(
    group.x >= outside.x + outside.width,
    "single-rank group must not cover an unrelated rank",
  );
  const labelBaseline = Number(
    result.svg.match(/<text\b[^>]*\by="([^"]+)"[^>]*>X{120}<\/text>/)?.[1],
  );
  assert.ok(
    labelBaseline <= nodeRect(result.svg, "node").y - 4,
    "group label must remain above its node",
  );
});
...
test("readable-v2 measures multi-row legends into intrinsic and explicit viewBox capacity", () => {
  ...
  workflow.meta.legend = {
    ...
    entries: Object.fromEntries(
      ["frontend", "backend", "security", "messagebus", "database", "cloud", "external"].map(
        (kind) => [kind, { label: `${kind} contract evidence` }],
      ),
    ),
  };
...
test("sanitized downstream fixtures cover labels, explicit routes, custom widths, and explicit viewBoxes", () => {
  ...
  const labels = compileSuccessfully(
    readJson(path.join(fixtureRoot, "labels.workflow.json")),
    "showcase",
  );
  assertRectInsideViewBox(
...
test("CLI validate workflow --layout-json prints the stable compiler receipt", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [cli, "validate", "workflow", input, "--layout-json", "--quality", "standard"],
    { encoding: "utf8" },
  );
...
test("CLI validate workflow --layout-json returns only the causal compiler failure receipt", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [cli, "validate", "workflow", input, "--layout-json", "--json"],
    { encoding: "utf8" },
  );
