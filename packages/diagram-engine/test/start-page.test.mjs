...
function executeStartPage(html) {
  ...
  const types = ["architecture", "workflow", "sequence", "dataflow", "lifecycle"].map(
    (type) => new FakeElement({ dataset: { type } }),
  );
  const agents = ["cursor", "codex", "claude-code", "opencode"].map(
    (agent) =>
      new FakeElement({ textContent: agent === "codex" ? "Codex" : agent, dataset: { agent } }),
  );
  const inputs = ["description", "repository"].map(
    (input) => new FakeElement({ dataset: { input } }),
  );
  const copySources = ["install-command", "project-command"].map(
    (copySource) => new FakeElement({ dataset: { copySource } }),
  );
  const copied = [];
  ...
  assert.ok(
    Object.values(data).every(
      (entry) =>
        entry.en.prompt &&
        entry.zh.prompt &&
        entry.en.descriptionPrompt &&
        entry.zh.descriptionPrompt &&
        entry.en.repositoryPrompt &&
        entry.zh.repositoryPrompt &&
        entry.proof,
    ),
  );
...
test("start page: canonical recipes own description and repository prompt variants", () => {
  ...
  for (const [type, id] of selected) {
    ...
    for (const language of ["en", "zh"]) {
      ...
      assert.ok(
        prompts.repositoryPrompt.toLowerCase().includes(recipe[language].prompt.toLowerCase()),
      );
    }
  }
});
...
test("start page: input mode drives rendered prompt, copy, keyboard, and URL without changing event schema", async () => {
  ...
  assert.equal(
    new URL(page.getUrl(), "https://example.test").searchParams.get("input"),
    "description",
  );
  ...
  assert.deepEqual(
    [viewEvent.step, promptEvent.step, starterEvent.step],
    ["start_view", "prompt_copy", "starter_copy"],
  );
});
