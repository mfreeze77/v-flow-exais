...
test("all five renderers expose one reader-controlled visual style picker", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.match(
      html,
      /id="btn-preset"[^>]+aria-haspopup="menu"[^>]+aria-controls="preset-menu"/,
      mode,
    );
    ...
    for (const preset of ["classic", "signal-flow", "blueprint", "editorial"]) {
      assert.match(
        html,
        new RegExp(`data-preset-value="${preset}"[^>]+role="menuitemradio"`),
        `${mode}: ${preset}`,
      );
    }
...
test("omitted visual preset opens as Classic and theme switching cannot change it", () => {
  const html = render("architecture");
  const themeRuntime =
    html.match(/Archify\.theme = \(function \(\) \{[\s\S]*?\n    \}\)\(\);/)?.[0] || "";
...
test("same topology keeps identical canonical SVG geometry across all four styles", () => {
  const normalize = (svg) =>
    svg.replace(/ data-preset="(?:classic|signal-flow|blueprint|editorial)"/, "");
  const variants = ["classic", "signal-flow", "blueprint", "editorial"].map((preset) =>
    normalize(svgBlock(render("architecture", preset))),
  );
