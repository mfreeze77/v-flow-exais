...
test("fixed column fit keeps the historical 108px gap regardless of viewBox width", () => {
  ...
  assert.equal(
    boxes.at(-1).x + boxes.at(-1).width < 600,
    true,
...
test("spread column fit uses the viewBox width and stays inside it", () => {
  ...
  assert.ok(
    boxes.at(-1).x + boxes.at(-1).width <= 1320 - 40,
...
test("a label the fixed box rejects fits the spread box on the same viewBox", () => {
  ...
  assert.ok(
    fixed.stderr.includes(`Label "${wideLabel}"`),
    `expected the label in stderr:\n${fixed.stderr}`,
  );
  assert.ok(
    fixed.stderr.includes("86px participant box"),
    `expected the fixed box width in stderr:\n${fixed.stderr}`,
  );
  ...
  assert.ok(
    estimatedLabelW <= box.width + 6,
    `label ~${estimatedLabelW}px must fit the ${box.width}px spread box`,
  );
