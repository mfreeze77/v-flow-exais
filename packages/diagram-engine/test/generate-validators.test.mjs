...
test("generated workflow validator accepts schema versions 1 and 2 only", () => {
  assert.equal(
    validateWorkflow(workflowDocument(1)),
    true,
    JSON.stringify(validateWorkflow.errors),
  );
  assert.equal(
    validateWorkflow(workflowDocument(2)),
    true,
    JSON.stringify(validateWorkflow.errors),
  );
...
test("validator freshness check accepts CRLF checkouts", () => {
  const scratch = fs.mkdtempSync(ownedSkillPath(skillRoot, ".validator-check-"));
  try {
    fs.mkdirSync(path.join(scratch, "scripts"));
    fs.mkdirSync(path.join(scratch, "renderers", "shared"), { recursive: true });
    fs.cpSync(ownedSkillPath(skillRoot, "schemas"), path.join(scratch, "schemas"), {
      recursive: true,
    });
    fs.copyFileSync(
      ownedSkillPath(skillRoot, "scripts", "generate-validators.mjs"),
      path.join(scratch, "scripts", "generate-validators.mjs"),
    );

    const validator = fs.readFileSync(
      ownedSkillPath(skillRoot, "renderers", "shared", "generated-validators.mjs"),
      "utf8",
    );
    fs.writeFileSync(
