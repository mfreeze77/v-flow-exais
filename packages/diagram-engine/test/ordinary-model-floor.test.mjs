...
test("benchmark rejects a renderer-valid candidate that changes required technical roles or relationship labels", () => {
  ...
  source.connections.find(
    (connection) => connection.from === "api" && connection.to === "db",
  ).label = "HTTP";
  ...
  const receipt = JSON.parse(result.stdout);
  assert.equal(
    receipt.gates.validation.ok,
    true,
    "the deterministic renderer should still accept this controlled drift",
  );
  ...
  const cases = [
    {
      ...
      nodes: [
        { id: "approval", type: "security" },
        { id: "tool", type: "messagebus" },
      ],
    ...
    },
    {
      ...
      nodes: [
        { id: "consent", type: "security" },
        { id: "pii", type: "security" },
      ],
    ...
    },
    {
      ...
      nodes: [
        { id: "approval", type: "waiting" },
        { id: "cancelled", type: "failure" },
      ],
...
test("benchmark semantic requirements bind by accepted technical labels instead of forcing model-authored internal IDs", () => {
  ...
  const caseFile = writeJson("semantic-aliases.case.json", {
    ...
    requirements: {
      ...
      relationships: [{ from: "api", to: "cache", labels: ["read-through", "cache read"] }],
    },
  });
...
test("checked-in cases accept equivalent ordinary-model vocabulary without weakening required topology", () => {
  ...
  const fixtures = [
    {
      ...
      mutate(candidate) {
        candidate.components.find((node) => node.id === "browser-clients").label =
          "Browser Clients";
    ...
    },
    {
      ...
      mutate(candidate) {
        ...
        Object.assign(
          candidate.nodes.find((node) => node.id === "risk-router"),
          { label: "Route Decision", type: "security" },
        );
        ...
        candidate.edges.find(
          (edge) => edge.from === "risk-router" && edge.to === "consent-check",
    ...
    },
    {
      ...
      mutate(candidate) {
        Object.assign(
          candidate.participants.find((node) => node.id === "browser-tab"),
          { label: "Browser", type: "external" },
        );
        ...
        candidate.messages.find((message) => message.id === "verify-jwt").label =
          "verify Bearer JWT";
        candidate.messages.find((message) => message.id === "cache-read").label =
          "GET dashboard key";
        candidate.messages.find((message) => message.id === "profile-query").label =
          "SELECT profile + metrics";
        candidate.messages.find((message) => message.id === "cache-write").label =
          "SETEX profile 300";
      },
    },
    {
  ...
  for (const fixture of fixtures) {
    ...
    const benchmarkCase = JSON.parse(
      fs.readFileSync(path.join(suiteRoot, "cases", fixture.caseFile), "utf8"),
    );
...
test("suite integrity rejects a long prompt that omits the attempt-1 file contract", () => {
  ...
  const receipt = JSON.parse(result.stdout);
  assert.equal(
    receipt.cases.find((item) => item.caseId === "web-runtime-architecture").promptOk,
    false,
  );
});
...
test("benchmark report marks only a complete first-pass matrix as evidence and rejects duplicate runs", () => {
  ...
  const rows = manifest.cases.map((entry) => {
    const benchmarkCase = JSON.parse(
      fs.readFileSync(path.resolve(path.dirname(manifestFile), entry.case), "utf8"),
    );
    return {
      schemaVersion: 1,
      benchmark: "ordinary-model-floor",
...
test("packaged skill puts a bounded ordinary-model path before progressive feature references", () => {
  ...
  assert.ok(
    fastPath < progressiveReferences,
    "fast authoring path must precede progressive references",
  );
  assert.ok(
    skill.trimEnd().split("\n").length <= 160,
    "ordinary authors must not ingest the viewer catalogue",
  );
  for (const required of [
    "one matching schema",
...
test("dated three-model evidence retains every frozen attempt-1 candidate and truthful gate result", () => {
  ...
  assert.equal(
    evidence.generation.packageSha256,
    "1f32354a466ec10c21f56346634ce66e170b6e0d23306cc2e9a9cbb68283f05a",
  );
...
test("post-fix evidence keeps the complete matrix and reports the no-uplift comparison truthfully", () => {
  ...
  assert.equal(
    evidence.generation.packageSha256,
    "cc34ab9484ca84e43fce9fce3de612b11c525c6dd5c797646645c6f35954b24e",
  );
  assert.equal(evidence.report.evidenceEligible, true);
  assert.deepEqual(evidence.report.overall, {
    runs: 15,
  ...
  const qwenArchitecture = evidence.runs.find(
    (entry) =>
      entry.model === "codewiz-anthropic/qwen3.7-plus" &&
      entry.caseId === "web-runtime-architecture",
  );
  assert.equal(qwenArchitecture.receipt.firstPassUsable, true);
  assert.equal(
    qwenArchitecture.run.visual_review.reviewer,
    "codex-browser-visual-audit-2026-07-26-route-fix",
  );

  const minimaxLifecycle = evidence.runs.find(
    (entry) =>
      entry.model === "codewiz-anthropic/minimax-m3" && entry.caseId === "agent-run-lifecycle",
  );
  ...
  assert.ok(
    minimaxLifecycle.receipt.gates.visualReview.defects.includes(
...
test("quality-first evidence preserves the complete matrix and the measured lifecycle gain without overstating uplift", () => {
  ...
  assert.equal(
    evidence.generation.packageSha256,
    "92135b360ee1502080dac8f2eea6258bb8fa0aa7f9a59cba119b02233f797593",
  );
  ...
  assert.equal(
    evidence.comparison.qualityFirst.firstPassUsableByCase["web-runtime-architecture"],
    3,
  );
  assert.match(evidence.comparison.outcome, /no measured overall uplift/i);
  ...
  assert.ok(
    minimaxLifecycle.candidate.states.some(
  ...
  const deepseekWorkflow = evidence.runs.find(
    (entry) =>
      entry.model === "seal/deepseek-v4-flash" && entry.caseId === "agent-tool-call-workflow",
  );
  assert.equal(deepseekWorkflow.receipt.firstPassUsable, false);
  assert.ok(
    deepseekWorkflow.receipt.gates.visualReview.defects.includes(
