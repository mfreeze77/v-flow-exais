...
test("all five renderers inherit one viewer-only Semantic Story Carrier", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /story-flow-token|story-carrier-token|semantic-flow-token/,
      mode,
    );
  }
});
...
test("Story deduplicates SVG path and label fragments by stable authored edge key", () => {
  ...
  assert.match(
    template,
    /!storyGeometry\(unique\[index\]\)\.length && storyGeometry\(edge\)\.length/,
  );
...
test("Story reuses the exact semantic token vocabulary on its existing finite edge pulse", () => {
  ...
  assert.match(
    template,
    /relationshipTokenGeometry\(shape, relationshipTokenKind\(edge\), key, options\)/,
  );
...
test("only explicit play=1 embeds may show the finite carrier", () => {
  assert.match(
    template,
    /data-embed'\) === 'true' &&\s*document\.documentElement\.getAttribute\('data-share-playback'\) !== 'true'/,
  );
  ...
  assert.match(
    template,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]+\.story-carrier-overlay \{ display: none !important; \}/,
  );
});
...
test("Story Carrier cleanup and export remain owned by Story Trail", () => {
  ...
  assert.equal(
    (template.match(/storyPulseOwnerToken = Archify\.motionGovernor\.claim\('story'/g) || [])
      .length,
    1,
  );
  assert.match(
  ...
  assert.match(
    template,
    /@media print \{[\s\S]+\.story-trail-overlay,[\s\S]+\.story-carrier-overlay \{ display: none !important; \}/,
  );
});
