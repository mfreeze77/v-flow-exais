...
test("the temporal hierarchy has one bounded next state and a clean final beat", () => {
  ...
  assert.match(
    template,
    /storyBeatIndex \+ 1 < storySteps\.length \? storySteps\[storyBeatIndex \+ 1\] : null/,
  );
  assert.match(
    template,
    /else \{\s*svg\.removeAttribute\('data-story-next'\);\s*panel\.removeAttribute\('data-story-next'\)/,
  );
...
test("next remains static, subordinate, preset-safe, and mobile-height neutral", () => {
  assert.match(
    template,
    /data-story-step\]\[data-story-beat-state="next"\] \{\s*opacity: 0\.5;\s*filter: saturate\(0\.66\)/,
  );
  assert.match(template, /data-story-beat-state="past"\] \{\s*opacity: 0\.72/);
  assert.match(
    template,
    /data-edge-from\]\[data-story-beat-step\]\[data-story-beat-state="next"\] \{ opacity: 0\.34; \}/,
  );
  assert.match(
    template,
    /guided-view-stop\[data-story-beat-state="next"\][\s\S]*border-style: dashed/,
  );
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-story-caption-next \{ display: none; \}/,
  );
  const nextRules =
...
test("Still, accessibility, teardown, and export preserve the product boundary", () => {
  ...
  assert.match(
    template,
    /html\[data-motion="still"\] svg \[data-node-id\][\s\S]*transition: none !important/,
  );
