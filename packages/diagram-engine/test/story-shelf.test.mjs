...
test("cold shelf keeps chapter identity and Play while removing only unavailable duplicate controls", () => {
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] > #guided-view-prev,[\s\S]*#guided-view-next,[\s\S]*\.guided-view-beat-link,[\s\S]*\.guided-view-all\s*\{\s*display: none;\s*\}/,
  );
  assert.doesNotMatch(
    template,
    /\.guided-views\[data-active-view="all"\][^{]*(?:\.guided-view-play|\.guided-view-index)[^{]*\{\s*display:\s*none/,
  );
...
test("desktop shelf follows DOM order and returns vertical space to the diagram", () => {
  ...
  assert.match(
    rule,
    /grid-template-columns:\s*minmax\(11rem,\s*\.72fr\)\s+auto\s+minmax\(0,\s*2fr\)/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-copy\s*\{[^}]*grid-column:\s*1/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-actions\s*\{[^}]*grid-column:\s*2/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-index\s*\{[^}]*grid-column:\s*3/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-copy > #guided-view-note\s*\{\s*display:\s*none/,
  );
});
...
test("mobile shelf preserves 44px controls, horizontal chapters, and honest expansion", () => {
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-views\[data-active-view="all"\][\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+auto/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-index\s*\{[^}]*grid-column:\s*1 \/ -1;[^}]*grid-row:\s*2/,
  );
  assert.match(
    template,
    /\.guided-views\[data-active-view="all"\] \.guided-view-play\s*\{[^}]*min-height:\s*2\.75rem/,
  );
...
test("active stories expand through existing state without storage or a second interaction owner", () => {
  assert.match(
    template,
    /panel\.setAttribute\('data-active-view', 'all'\);\s*panel\.hidden = false/,
  );
  ...
  assert.doesNotMatch(
    template,
    /storyShelf(?:Open|Expanded|Storage)|archify-story-shelf|localStorage[^\n]*shelf/i,
  );
});
