...
function shieldsBadgeMessages(source, label) {
  ...
  while (searchFrom < source.length) {
    ...
    try {
      messages.push(decodeURIComponent(message));
    } catch {
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(
      `${mode}: packaged ${golden}`,
      normalizeNewlines(fresh) === normalizeNewlines(packaged),
...
for (const tag of ["style", "script"]) {
  ...
  const t = blocks(template, tag).filter(
    (b) => !b.includes("[PROJECT NAME]") && isTemplateOwned(b),
  );
  const w = blocks(webApp, tag).filter((b) => !b.includes("Sample Web App") && isTemplateOwned(b));
  check(
    `web-app.html ${tag} blocks match template`,
...
check(
  "GitHub Pages version labels match package.json",
