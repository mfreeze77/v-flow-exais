...
const options = {
  skip: chrome ? false : "Set ARCHIFY_CHROME for offline font and export browser acceptance.",
};
...
async function actualFonts(browser, selector) {
  ...
  const { nodeId } = await browser.cdp.send(
    "DOM.querySelector",
...
test(
  "fresh viewers use bundled fonts with local fonts disabled and identical offline layout",
  options,
  async () => {
    ...
    try {
      ...
      for (const blocked of [false, true]) {
        ...
        try {
          ...
          for (const name of ["production", "web", "mixed"])
            for (const theme of ["light", "dark"]) {
              const metrics = await browser.inspect({
                artifactPath: path.join(tmp, `${name}.html`),
                width: 1440,
                height: 900,
                theme,
              });
              const snapshot = {
                readerWidth: metrics.readerWidth,
                diagramWidth: metrics.diagramWidth,
              ...
              const fonts = await actualFonts(
                browser,
                ".diagram-container svg text[data-node-label]",
              );
              assert.ok(
                fonts.some((f) => f.isCustomFont && /JetBrains Mono/.test(f.familyName)),
          ...
          const frames = await evaluate(
            browser,
            `(async () => Promise.all(Array.from(document.querySelectorAll('iframe[srcdoc]')).map(async frame => {
...
test(
  "SVG and raster exports preserve the viewer font with local fonts and network disabled",
  options,
  async () => {
    ...
    try {
      ...
      for (const format of ["png", "jpeg", "webp", "share-card"]) {
        ...
        if (format === "share-card")
          assert.deepEqual([result.width, result.height], ["1200", "630"]);
      }
      ...
      const pixelsDiffer = await evaluate(
        browser,
        `(async () => {
      ...
      await browser.cdp.send(
        "Page.navigate",
        { url: pathToFileURL(path.join(tmp, "export.svg")).href },
        session,
      );
      await loaded;
...
for (const format of ["png", "share-card"])
  test(
    `${format} requested at DOMContentLoaded matches a font-settled repeat`,
    options,
    async (t) => {
    ...
    `,
      ...
      return current.length === window.__firstCardBytes.length && current.every((byte,i) => byte === window.__firstCardBytes[i]);
        ...
        t.diagnostic(
          `${format}: fonts at initial click: ${early.statusAtClick}; fonts ready for every Canvas text draw`,
        );
        assert.deepEqual(requests, []);
      } finally {
        await browser.close();
