...
test("focus pauses without selection while native activation pins one beat and preserves chapter and link ownership", () => {
  assert.match(
    template,
    /trail\.addEventListener\('focusin'[\s\S]*if \(playing\) pausePlayback\(\)/,
  );
  assert.match(
    template,
    /trail\.addEventListener\('click'[\s\S]*selectStoryBeat\(Number\(stop\.getAttribute\('data-story-index'\)\)\)/,
  );
  assert.match(
    template,
    /trail\.addEventListener\('keydown'[\s\S]*event\.key !== 'Enter' && event\.key !== ' '[\s\S]*event\.preventDefault\(\)/,
  );
...
test("one generation-owned scheduler resumes remaining dwell and finite exact-edge signals never loop", () => {
  ...
  assert.match(
    template,
    /svg\[data-preset="blueprint"\]\[data-story-beat\] \[data-story-step\]\[data-story-beat-state="active"\] \{\s*filter: none;\s*animation: none;/,
  );
...
test("target size, reduced motion, print, embed, and export keep Story Beats viewer-only", () => {
  ...
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-view-trail \.guided-view-stop \{[\s\S]*min-height: 2rem/,
  );
  assert.match(template, /touch-action: pan-x/);
  assert.match(
    template,
    /document\.documentElement\.getAttribute\('data-embed'\) !== 'true' && typeof MutationObserver !== 'undefined' && typeof Node !== 'undefined' && svg instanceof Node && document\.documentElement instanceof Node/,
  );
  assert.match(
    template,
    /@media print[\s\S]*\.story-trail-overlay,\s*\.story-carrier-overlay \{ display: none !important; \}/,
  );
  ...
  assert.match(
    template,
    /clone\.querySelectorAll\('\[data-story-step\], \[data-story-beat-state\], \[data-story-beat-step\]'\)/,
  );
