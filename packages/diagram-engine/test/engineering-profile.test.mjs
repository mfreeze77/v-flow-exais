...
test("deployment ownership profile reports exact owners, scopes, state, and crossing mechanisms", () => {
  ...
  candidate.boundaries.find((boundary) => boundary.label.includes("us-east-1")).wraps =
    candidate.boundaries
      .find((boundary) => boundary.label.includes("us-east-1"))
      .wraps.filter((id) => id !== "edge");
  candidate.boundaries.find((boundary) => boundary.label === "private application network").wraps =
    candidate.boundaries
      .find((boundary) => boundary.label === "private application network")
      .wraps.filter((id) => id !== "redis");
  const crossing = candidate.connections.find(
    (connection) => connection.from === "gateway" && connection.to === "api_a",
  );
  crossing.label = "";
  ...
  const boundaryDiagnostic = diagnostics.find(
    (entry) => entry.code === "engineering/deployment-crossing-mechanism",
  );
  ...
  assert.ok(
    boundaryDiagnostic.evidence.crossedBoundaries.some(
      (boundary) => boundary.kind === "security-group",
    ),
  );
...
test("deployment ownership profile requires both region and private boundary kinds", () => {
  ...
  assert.ok(
    diagnostics.some(
      (entry) =>
        entry.code === "engineering/deployment-boundary-kind" &&
...
test("deployment ownership profile rejects ambiguous regions and cross-region private groups", () => {
  const candidate = clone(example);
  const secondRegion = candidate.boundaries.find((boundary) =>
    boundary.label.includes("eu-west-1"),
  );
  ...
  assert.ok(
    diagnostics.some(
      (entry) =>
        entry.code === "engineering/deployment-region-ambiguous" && entry.subject.id === "api_a",
    ),
  );
  assert.ok(
    diagnostics.some(
      (entry) =>
        entry.code === "engineering/deployment-private-region-consistency" &&
...
test("deployment ownership crossing math follows authored membership instead of geometry or labels", () => {
  ...
  for (const [name, from, to] of cases) {
    ...
    connection.label = "";
    const crossings = deploymentOwnershipDiagnostics(candidate).filter(
      (diagnostic) => diagnostic.code === "engineering/deployment-crossing-mechanism",
    );
  ...
  sameScope.connections.find(
    (connection) => connection.from === "api_a" && connection.to === "redis",
  ).label = "";
  sameScope.connections.push({ from: "api_a", to: "api_a", label: "" });
  assert.ok(
    !deploymentOwnershipDiagnostics(sameScope).some(
      (diagnostic) => diagnostic.code === "engineering/deployment-crossing-mechanism",
    ),
  );
});
...
test("other diagram modes reject the architecture-only engineering profile", () => {
  ...
  try {
    for (const [mode, fixture] of fixtures) {
      ...
      assert.ok(
        receipt.diagnostics.some((diagnostic) => diagnostic.code === "schema/additionalProperties"),
        mode,
      );
    }
  } finally {
...
test("validate and deliver expose one truthful engineering-profile receipt", () => {
  ...
  try {
    ...
    const crossing = invalid.connections.find(
      (connection) => connection.from === "gateway" && connection.to === "api_a",
    );
    crossing.label = "";
    ...
    const failedDelivery = spawnSync(
      process.execPath,
    ...
    const validated = spawnSync(
      process.execPath,
      [cli, "validate", "architecture", examplePath, "--json"],
      {
        cwd: tmp,
        encoding: "utf8",
      },
    ...
    const delivered = spawnSync(
      process.execPath,
      [cli, "deliver", "architecture", examplePath, output, "--json"],
      {
        cwd: tmp,
        encoding: "utf8",
      },
