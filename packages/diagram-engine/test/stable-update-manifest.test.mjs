...
function annotatedTaggerTime(root, tag) {
  const result = git(root, ["for-each-ref", "--format=%(taggerdate:unix)", `refs/tags/${tag}`]);
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout.trim(), /^\d+$/);
  return new Date(Number(result.stdout.trim()) * 1_000).toISOString().replace(".000Z", "Z");
}
...
function runCheck(root, archive, extraArguments = []) {
  return spawnSync(
    process.execPath,
    [checker, "--root", root, "--archive", archive, "--tag", "v3.0.0", ...extraArguments],
    { encoding: "utf8" },
  );
}
  ...
  try {
    ...
    assert.equal(
      git(fixture, [
        ...
        "-c",
        "user.email=archify@example.invalid",
        "commit",
        "-m",
        "stable fixture",
      ]).status,
    ...
    writeJson(manifestPath, {
      ...manifest,
      publishedAt: new Date(Date.parse(publishedAt) + 1_000).toISOString().replace(".000Z", "Z"),
    });
