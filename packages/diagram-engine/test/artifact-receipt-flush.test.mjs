...
for (const profile of ["standard", "showcase"]) {
  test(`artifact checker flushes a large ${profile} receipt to a pipe`, () => {
    ...
    fs.writeFileSync(
      html,
      `<svg viewBox="0 0 240 160" data-quality-profile="${profile}">${arrows}</svg>`,
    );
    ...
    assert.ok(
      receipt.composition.issues.every(
        (issue) =>
    ...
    assert.equal(
      Buffer.byteLength(piped.stdout),
      Buffer.byteLength(expected),
      "pipe must not truncate the receipt",
    );
