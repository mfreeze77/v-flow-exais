...
test("the embedded update identity must match the package release exactly", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /archify\/skill-release\.json must identify archify 2\.13\.0-dev\.0 as development/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("stable public-page templates reject development labels on version-bearing fallbacks", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /scripts\/guide-template\.html must not label \[\[ARCHIFY_VERSION\]\] as development or 开发版/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
