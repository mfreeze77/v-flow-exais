...
test("export menu has grouped, single-column rows and a zoom-safe width", () => {
  ...
  assert.match(
    template,
    /class="export-menu-section" role="group" aria-label="\{\{i18n:viewer\.export\.share\}\}"/,
  );
  assert.match(
    template,
    /class="export-menu-section" role="group" aria-label="\{\{i18n:viewer\.export\.raster\}\}"/,
  );
  assert.match(
    template,
    /class="export-menu-section" role="group" aria-label="\{\{i18n:viewer\.export\.vectorMotion\}\}"/,
  );
...
test("diagram view reset separates semantic detail from zoom percentage", () => {
  assert.match(
    template,
    /data-view="reset"[\s\S]*?data-view-detail hidden>\{\{i18n:viewer\.nav\.read\}\}<[\s\S]*?data-view-percent>100%</,
  );
  assert.match(
    template,
