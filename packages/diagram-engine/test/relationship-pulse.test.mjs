...
test("all typed renderers inherit one exact-edge Directional Flow Pulse", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /relationship-flow-(?:pulse|token)|data-relationship-(?:pulse|token)/,
      mode,
    );
  }
});
...
test("semantic token classification is evidence-based and fail-closed", () => {
  ...
  assert.match(
    html,
    /a-security.+sourceKind === 'security'.+targetKind === 'failure'.+return 'security'/s,
  );
  assert.match(
    html,
    /a-dashed.+sourceKind === 'messagebus'.+targetKind === 'messagebus'.+return 'event'/s,
  );
...
test("semantic tokens use five distinct inline SVG cues on one finite timing owner", () => {
  ...
  assert.match(
    html,
    /kind === 'data'[\s\S]+kind === 'event'[\s\S]+kind === 'security'[\s\S]+kind === 'state'/,
  );
...
test("pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe", () => {
  ...
  assert.match(
    html,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]+\.relationship-pulse-overlay \{ display: none !important; \}/,
  );
});
...
test("pulse has one owner and never enters print or canonical exports", () => {
  ...
  assert.match(
    html,
    /@media print \{[\s\S]+\.relationship-pulse-overlay \{ display: none !important; \}/,
  );
