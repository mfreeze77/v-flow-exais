...
test("Gitee evidence generates provider-specific revision and line links", () => {
  ...
  for (const remote of [
    ...
    assert.equal(
      evidence.repository.href,
      `https://gitee.com/example/evidence-repo/tree/${data.revision}`,
    );
    assert.equal(
      evidence.nodes.users[0].href,
      `https://gitee.com/example/evidence-repo/blob/${data.revision}/src/router.js#L1-3`,
    );
    assert.equal(
      evidence.nodes.users[1].href,
      `https://gitee.com/example/evidence-repo/blob/${data.revision}/src/store.js#L1`,
    );
  }
});
...
test("local-only supports nested HTTPS and Git SSH identities with bounded port equivalence", () => {
  ...
  for (const [url, remote] of [
    [
      "https://git.internal/Platform/Services/repo.git",
      "https://git.internal:443/Platform/Services/repo.git",
    ],
    [
      "ssh://git@git.internal/Platform/Services/repo",
      "ssh://git@git.internal:22/Platform/Services/repo",
    ],
    [
    ...
    [
      "http://git.internal:3000/Platform/repo.git",
      "http://user:SYNTHETIC_TOKEN@git.internal:3000/Platform/repo.git",
    ],
    [
      "https://git.internal/Platform/repo.git",
      "https://user:SYNTHETIC_TOKEN@git.internal/Platform/repo.git",
    ],
...
for (const [name, url, origin] of [
  [
    "relative versus absolute SSH paths",
    "ssh://git@git.internal/Team/repo",
    "git@git.internal:Team/repo",
  ],
  [
    "literal percent escapes in SCP paths",
    "git@git.internal:Team/repoA",
    "git@git.internal:Team/repo%41",
  ],
]) {
  test(`local-only rejects ${name} before replacing a trusted artifact`, () => {
    ...
    assert.equal(
      result.status,
      1,
      "different Git paths must not verify as the declared repository",
    );
    assert.ok(
      JSON.parse(result.stdout).diagnostics.some(
        ({ code }) => code === "repository-evidence/origin-mismatch",
      ),
    );
...
test("local-only preserves root, origin, commit, blob, path and line checks", () => {
  ...
  const cases = [
    ...
    [
      ...
      (diagram) => {
        diagram.meta.repository.revision = "0".repeat(40);
      },
    ],
    [
      "repository-evidence/file-missing",
      (diagram) => {
        diagram.components[0].sources = [{ path: "src/missing.js" }];
      },
    ],
    [
      "repository-evidence/file-missing",
      (diagram) => {
        diagram.components[0].sources = [{ path: "src" }];
      },
    ],
    [
      "repository-evidence/path-escape",
      (diagram) => {
        diagram.components[0].sources = [{ path: "../outside.js" }];
      },
    ],
    [
      "repository-evidence/path-escape",
      (diagram) => {
        diagram.components[0].sources = [{ path: ".git/config" }];
      },
    ],
    [
      "repository-evidence/line-out-of-range",
      (diagram) => {
        diagram.components[0].sources = [{ path: "src/router.js", line: 4 }];
      },
    ],
    [
      "repository-evidence/line-range-invalid",
      (diagram) => {
        diagram.components[0].sources = [{ path: "src/router.js", line: 3, end_line: 1 }];
      },
    ],
  ];
  for (const [expectedCode, change, roots = ["--repo-root", data.root]] of cases) {
    ...
    assert.ok(
      JSON.parse(result.stdout).diagnostics.some(({ code }) => code === expectedCode),
      result.stdout,
    );
    assert.equal(fs.readFileSync(output, "utf8"), "trusted previous artifact");
  }
...
test("explicit providers retain GitHub links and encode Gitee source paths", () => {
  ...
  for (const provider of ["github", "gitee"]) {
    data.diagram.meta.repository = {
      url: `https://${provider}.com/example/evidence-repo.git/`,
      revision,
      provider,
    };
    ...
    assert.equal(
      evidence.nodes.users[0].href,
      `https://${provider}.com/example/evidence-repo/blob/${revision}/src/${encodeURIComponent("中文 # router.js")}#L1-${provider === "github" ? "L" : ""}2`,
    );
  }
});

test(
  "local-only preview and compare publish only revision-verified evidence",
  { timeout: 20000 },
  async () => {
    const data = fixture();
    data.diagram.meta.repository.url = "http://git.internal/Team/repo";
    ...
    const preview = await startPreview({
      type: "architecture",
      input: data.input,
      output: path.join(data.root, "preview-local.html"),
      repoRoot: data.root,
      open: false,
      debounceMs: 30,
      pollMs: 60,
...
test(
  "browser renders local-only sources as searchable text and web sources as links",
  {
    ...
    try {
      for (const mode of ["github", "gitee", "local-only"]) {
        ...
        const url = local
          ...
          : `https://${mode}.com/example/evidence-repo`;
        data.diagram.meta.repository = {
          url,
          revision: data.revision,
          ...(local ? { link_mode: mode } : {}),
        };
...
test("origin-mismatch diagnostics redact HTTPS remote userinfo", () => {
  ...
  git(
    ...
    "set-url",
    "origin",
    "https://user:not-a-real-token@github.com/example/other-repo.git",
  );
  const result = run([
    "deliver",
    "architecture",
  ...
  const diagnostic = receipt.diagnostics.find(
    (entry) => entry.code === "repository-evidence/origin-mismatch",
  );
  ...
  assert.match(
    diagnostic.evidence.localOrigin,
    /^https:\/\/REDACTED@github\.com\/example\/other-repo\.git$/,
  );
  assert.equal(fs.readFileSync(output, "utf8"), "trusted previous artifact");
});
...
test("validate accepts credentialed HTTPS remotes and redacts mismatch diagnostics", () => {
  ...
  git(
    ...
    "set-url",
    "origin",
    "https://user:VALIDATE_SECRET@github.com/example/other-repo.git",
  );
...
test(
  "preview accepts credentialed HTTPS remotes and redacts mismatch diagnostics",
  { timeout: 20000 },
  async () => {
    const data = fixture();
    ...
    git(
      ...
      "set-url",
      "origin",
      "https://user:PREVIEW_SECRET@github.com/example/other-repo.git",
    );
