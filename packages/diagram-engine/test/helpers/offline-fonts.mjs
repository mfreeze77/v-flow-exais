...
const EXPECTED_FACES = [
  [
    "9343de2ca5d9549f792e7962375af8efb0f320c7643bfd36c884b5a30e5c396f",
    "U+0460-052F,U+1C80-1C8A,U+20B4,U+2DE0-2DFF,U+A640-A69F,U+FE2E-FE2F",
  ],
  [
    "4995a9a43ac659ec32fcd8b463755cd6a07b31a6e6b3894a6a153b661cf490e2",
    "U+0301,U+0400-045F,U+0490-0491,U+04B0-04B1,U+2116",
  ],
  [
    "49c3da6c9a2b279b0f1f860f5cfb1f5dc38d88a5c7be9c9b1837bbc4e3db6111",
    "U+0370-0377,U+037A-037F,U+0384-038A,U+038C,U+038E-03A1,U+03A3-03FF",
  ],
  [
    "d44eb1936043a56038eb02dd70b243f379bef65783f94ec12f277550720411f1",
    "U+0102-0103,U+0110-0111,U+0128-0129,U+0168-0169,U+01A0-01A1,U+01AF-01B0,U+0300-0301,U+0303-0304,U+0308-0309,U+0323,U+0329,U+1EA0-1EF9,U+20AB",
  ],
  [
    "9c38cb2d0d2d93c1ee6e21fa78db76f13ea7e15e15cc64214c7ca89b6aaa35c4",
...
export function assertFontCss(css, subject) {
  ...
  const faces = [...clean.matchAll(/@font-face\s*\{([^}]+)\}/gi)]
    .map(([, block]) => {
      const descriptor = (name) =>
        block.match(new RegExp(`\\b${name}\\s*:\\s*([^;]+)`, "i"))?.[1].trim();
      ...
      assert.doesNotMatch(
        block,
        /\blocal\s*\(/i,
        `${subject}: installed fonts must not override embedded bytes`,
      );
      const encoded = block.match(
        /\bsrc\s*:\s*url\(\s*["']?data:font\/woff2;base64,([A-Za-z0-9+/=]+)["']?\s*\)/i,
      )?.[1];
...
export function inspectDocuments(html, subject = "artifact") {
  ...
  function cssResources(css) {
    ...
    for (const match of clean.matchAll(
      /(?:url\(\s*|@import\s+)["']?((?:https?:)?\/\/[^"')\s;]+)/gi,
    ))
      document.resources.push(match[1]);
  }
  function visit(node) {
    ...
    for (const name of ["src", "poster", "data"])
      if (remote(attrs[name])) document.resources.push(attrs[name]);
    if (attrs.srcset)
      for (const match of attrs.srcset.matchAll(/(?:^|[\s,])((?:https?:)?\/\/[^\s,]+)/g))
        document.resources.push(match[1]);
    if (
      ["image", "use", "feImage"].includes(node.tagName) ||
      (node.tagName === "link" &&
        /\b(stylesheet|preconnect|dns-prefetch|preload|modulepreload|prefetch|icon)\b/.test(
          attrs.rel || "",
        ))
    ) {
      if (remote(attrs.href)) document.resources.push(attrs.href);
    }
    if (node.tagName === "iframe" && attrs.srcdoc != null)
      document.children.push(
        ...inspectDocuments(attrs.srcdoc, `${subject}/srcdoc[${document.children.length}]`),
      );
