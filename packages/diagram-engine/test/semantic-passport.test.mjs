...
test("renderer-owned structure supplies truthful Semantic Passport context", () => {
  ...
  assert.match(
    workflow,
    /data-node-id="approval"[^>]+data-node-kind="security"[^>]+data-node-context="Policy &amp; Recovery › Human or policy stop › Plan \+ route"/,
  );
  assert.match(
    sequence,
...
test("Node Finder searches and presents the same passport facts", () => {
  ...
  assert.match(
    html,
    /\[viewerKindLabel\(item\.type\), item\.id, item\.sublabel, item\.tag\]\.filter\(Boolean\)\.join\(' \\u00b7 '\)/,
  );
  assert.match(
    html,
