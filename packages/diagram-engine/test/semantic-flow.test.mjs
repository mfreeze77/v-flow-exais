...
test("all typed renderers inherit one selection-triggered Semantic Flow signal", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /semantic-lens-flow|semantic-lens-overlay|data-lens-flow/,
      mode,
    );
  }
});
...
test("Semantic Flow has preset identities and motion-safe density boundaries", () => {
  ...
  assert.match(
    html,
    /\.semantic-lens-flow\[data-direction="out"\],[\s\S]+var\(--frontend-stroke\)/,
  );
  assert.match(html, /\.semantic-lens-flow\[data-direction="in"\],[\s\S]+var\(--database-stroke\)/);
  assert.match(
    html,
    /\.semantic-lens-flow\[data-direction="within"\][\s\S]+var\(--messagebus-stroke\)/,
  );
  ...
  assert.match(
    html,
    /@media print \{[\s\S]+\.semantic-lens-overlay \{ display: none !important; \}/,
  );
  assert.match(
    html,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]+\.semantic-lens-flow \{[\s\S]+animation: none !important/,
  );
});
