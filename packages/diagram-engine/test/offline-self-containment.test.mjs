...
test("font checks accept equivalent CSS but reject missing bytes, coverage and local overrides", () => {
  ...
  const reordered = css.replaceAll(
    "font-family: 'JetBrains Mono'; font-style: normal;",
    'font-style:normal; font-family:"JetBrains Mono";',
  );
  ...
  assert.throws(() =>
    assertFontCss(css.replace("U+0460-052F", "U+0460-052E"), "missing character"),
  );
  assert.throws(() =>
    assertFontCss(css.replace("src: url(", "src: local('JetBrains Mono'), url("), "local override"),
  );
});

test("each compare srcdoc must carry its own font and reject external resources", () => {
  const frame = (html) =>
    `<iframe srcdoc="${html.replaceAll("&", "&amp;").replaceAll('"', "&quot;").replaceAll("<", "&lt;")}"></iframe>`;
  const emptyFont = template.replace(/@font-face\s*\{[^}]+\}/g, "");
  assert.equal(assertOfflineArtifact(frame(template) + frame(template), "compare"), 2);
  assert.throws(
    () => assertOfflineArtifact(frame(template) + frame(emptyFont), "compare"),
    /srcdoc\[1\]/,
  );
  for (const resource of [
    '<link rel="stylesheet" href="//fonts.example/font.css">',
    '<style>@import "https://fonts.example/font.css";</style>',
    '<img srcset="https://images.example/1.png 1x, https://images.example/2.png 2x">',
  ]) {
    assert.throws(
      () => assertOfflineArtifact(frame(template) + frame(template + resource), "compare"),
      /external subresource/,
    );
  }
  assertOfflineArtifact(
    template +
      '<!-- https://github.com/JetBrains/JetBrainsMono --><a href="https://github.com/JetBrains/JetBrainsMono">Source</a>',
    "attribution",
  );
});
...
test("every checked-in viewer artifact carries its font and reaches no external origin", () => {
  ...
  const tracked = spawnSync(
    "git",
    ["ls-files", "-z", "--", "archify/examples", "docs", "examples"],
    { cwd: repoRoot, encoding: "utf8" },
  );
  assert.equal(tracked.status, 0, tracked.stderr);
  const artifacts = tracked.stdout
    .split("\0")
  ...
  for (const required of [
    ...
    ...DIAGRAMS.map(([, , output]) => `archify/examples/${output}`),
  ]) {
