...
test("zh-CN localizes renderer-owned output across all five modes without translating authored content", () => {
  ...
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.ok(
      result.html.includes(`<title>${authoredTitle}</title>`),
      `${type}: authored title changed`,
    );
    assert.ok(
      result.html.includes(`<h1>${authoredTitle}</h1>`),
    ...
    assert.match(
      result.html,
      new RegExp(`<desc id="archify-diagram-description">\u7531 Archify \u751f\u6210\u7684`),
    );
...
test("explicit en and zh-CN preserve complete authored field inventories across all five modes", () => {
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.equal(
      english.authored.length,
      chinese.authored.length,
      `${type}: authored shapes differ`,
    );
    ...
    for (const candidate of [english, chinese]) {
      ...
      for (const authoredText of candidate.authored) {
        assert.ok(
          result.html.includes(authoredText),
          `${type}/${locale}: lost authored text ${authoredText}`,
        );
      }
      if (locale === "zh-CN") {
        ...
        assert.ok(
          result.html.includes(`<title>${candidate.document.meta.title} Diagram</title>`),
          type,
        );
...
test("omitted locale preserves non-English authored content and the English Viewer contract in all five modes", () => {
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.match(result.html, /^<!DOCTYPE html>\n<html lang="en"/);
    assert.ok(
      result.html.includes(`<title>${authoredTitle} Diagram</title>`),
      `${type}: authored title changed`,
    );
    assert.ok(
      result.html.includes(`<h1>${authoredTitle}</h1>`),
  ...
  for (const locale of ["fr", "zh-HK"]) {
    for (const type of Object.keys(EXAMPLES)) {
      ...
      assert.notEqual(
        result.status,
        0,
        `${type}: unsupported locale ${locale} unexpectedly passed`,
      );
      const payload = JSON.parse(result.stdout);
      assert.equal(payload.ok, false);
      assert.ok(
        payload.diagnostics.some((entry) => entry.subject?.path === "/meta/locale"),
        `${type}: ${locale}`,
      );
    }
  }
...
test(
  "real Chrome keeps zh-CN Finder, Route, Export, and accessibility UI localized in all five modes",
  {
    skip: chromePath
      ? false
      : "Set ARCHIFY_CHROME to run the real browser localization regression.",
  },
  async () => {
    ...
    try {
      for (const type of Object.keys(EXAMPLES)) {
        ...
        const shareCardFailure = await evaluate(
          browser,
          sessionId,
          `(async function () {
...
test("Share Card and export failures use catalog messages instead of fixed English", () => {
  assert.equal(
    translateCount("zh-CN", "viewer.export.card.routeSummary", 2, {
      source: "来源",
      target: "目标",
    }),
    "路径：来源 → 目标 · 2 个有向跳转",
