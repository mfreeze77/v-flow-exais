...
test("site language runtime normalizes one entry parameter into one durable preference", () => {
  ...
  for (const legacyKey of ["archify-gallery-language", "archify-guide-language"]) {
    ...
    assert.equal(
      legacy.language.read(),
      "zh",
      `${legacyKey} must remain readable during migration`,
    );
    assert.equal(
      legacy.values.get("archify-lang"),
      "zh",
      `${legacyKey} must migrate to the canonical key`,
    );
  }
  ...
  assert.equal(
    migrated.language.read(),
    "zh",
    "the canonical migration must win on later page loads",
  );
...
test(
  "custom site builders emit every shared site asset and preserve entry, navigation, selection, and refresh state",
  {
  ...
  () => {
    ...
    try {
      const builds = [
        {
          script: "build-start.mjs",
          args: [path.join(tmp, "start-site/start.html")],
          root: "start-site",
        },
        {
          script: "build-guide.mjs",
          args: [path.join(tmp, "guide-site/guide.html")],
          root: "guide-site",
        },
        {
          script: "build-gallery.mjs",
          args: [path.join(tmp, "gallery-site")],
          root: "gallery-site",
        },
      ];
      ...
      const refreshed = loadRuntime({
        url: explicitEnglish.url().href,
        values,
        source: emittedRuntime,
      });
      assert.equal(refreshed.language.read(), "zh");

      const nextPage = loadRuntime({
        url: "https://example.test/gallery.html",
        values,
        source: emittedRuntime,
      });
      ...
      const refreshedEnglish = loadRuntime({
        url: nextPage.url().href,
        values,
        source: emittedRuntime,
      });
...
test("all site pages consume one language runtime and one navigation contract", () => {
  ...
  for (const relative of pages) {
    ...
    assert.match(
      ...
      `${relative}: shared runtime missing`,
    );
    assert.match(
      html,
      /<link rel="stylesheet" href="assets\/site-navigation\.css">/,
      `${relative}: shared navigation missing`,
    );
    assert.match(
      html,
      /<nav class="site-nav" aria-label="Primary navigation">/,
      `${relative}: canonical navigation root missing`,
    );
    assert.match(html, /ArchifySiteLanguage\.read\(/, `${relative}: shared language read missing`);
    assert.match(
      html,
      /ArchifySiteLanguage\.write\(/,
      `${relative}: shared language write missing`,
    );
    ...
    assert.doesNotMatch(
      html,
      /(?:^|\s)nav\s*\{/,
      `${relative}: inline navigation layout bypasses the shared contract`,
    );
    assert.doesNotMatch(
      html,
      /\.nav-right\s*\{/,
      `${relative}: inline navigation actions bypass the shared contract`,
    );
    assert.doesNotMatch(
      html,
...
test("site page identity paths localize with the selected language", () => {
  const pages = [
    ...
    {
      paths: ["scripts/gallery-template.html", "docs/gallery.html"],
      en: "/ proof lab",
      zh: "/ 验证作品集",
    },
...
test("proof gallery type filters localize with the selected language", () => {
  ...
  for (const filter of filters) {
    ...
    assert.ok(
      template.includes(
        `data-filter="${filter.type}" aria-pressed="false" data-en="[[DIAGRAM_TYPE_${placeholder}_EN]]" data-zh="[[DIAGRAM_TYPE_${placeholder}_ZH]]"`,
      ),
  ...
  for (const relative of ["docs/gallery.html"]) {
    ...
    for (const filter of filters) {
      ...
      assert.match(
        html,
        bilingualFilter,
        `${relative}: ${filter.type} filter must expose English and Chinese copy`,
      );
    }
    assert.match(
      html,
...
test(
  "real Chrome preserves language through entry, navigation, selection, refresh, and consistent navigation chrome",
  {
  ...
  async () => {
    ...
    try {
      ...
      await navigate(
        browser,
        sessionId,
        `${baseUrl}/index.html?lang=zh&utm_source=browser-test#proof`,
      );
      ...
      assert.equal(
        await evaluate(browser, sessionId, 'document.querySelector(".nav-logo-path").textContent'),
        "/ 验证作品集",
      );
      assert.deepEqual(
        await evaluate(
          browser,
          sessionId,
          `Array.from(document.querySelectorAll('[data-filter]')).map(function (button) {
      ...
      assert.deepEqual(state, {
        language: "zh-CN",
        selected: "true",
        typeQuery: "architecture",
        visibleCount: 2,
        onlyArchitecture: true,
      });
      ...
      assert.deepEqual(
        await evaluate(
          browser,
          sessionId,
          `Array.from(document.querySelectorAll('#filters [data-filter]')).map(function (button) {
      return button.textContent;
    })`,
        ),
      ...
      await evaluate(
        browser,
        sessionId,
        "document.querySelector('#filters [data-filter=\"sequence\"]').click()",
      );
      state = await evaluate(
        browser,
        sessionId,
      ...
      assert.equal(await evaluate(browser, sessionId, "document.documentElement.lang"), "zh-CN");
      assert.equal(
        await evaluate(browser, sessionId, 'new URL(location.href).searchParams.has("lang")'),
        false,
      );
      assert.equal(
        await evaluate(browser, sessionId, 'document.querySelector(".nav-logo-path").textContent'),
      ...
      for (const page of pages) {
        ...
        assert.deepEqual(
          mobile,
          { height: 60, left: 0, right: 390, actionsRight: 370, linkDisplay: "none" },
          page,
        );
      }
    } finally {
