...
test(
  "Dock Safe Rail keeps typed renderers clear across themes, Presentation, and low-height desktops",
  {
...
test(
  "an artifact with no Legend still receives the desktop stage rail",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Dock Safe Rail resolves a forced Legend collision across the shared diagram viewer",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Maka remains collision-free at the reported Retina-equivalent viewport",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "a real 5px Legend gap keeps the stage rail and Legend clear",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Presentation keeps its visible Dock clear of a colliding Legend",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "manual zoom and pan reschedules Legend and Dock collision measurement",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "camera pan clips authored relationship paint at the protected stage boundary",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "live camera transitions keep authored relationship paint outside the Dock on every frame",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "zoom keeps the desktop rail stable and reports protected stage geometry",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    try {
      ...
      assert.ok(
        Math.abs(zoomed.reserve - baseline.reserve) <= 1,
...
test(
  "Reset followed immediately by zoom and pan retains a collision-free desktop rail",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "zoomed camera restores its bounded desktop rail after crossing the mobile breakpoint",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    try {
      ...
      assert.ok(
        Math.abs(restored.reserve - baseline.reserve) <= 1,
        JSON.stringify({ baseline, zoomed, restored }),
      );
...
test(
  "localized multiline Legends remain clear across required viewports, themes, and presets",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Semantic Lens and Radar protect the final Legend and Dock rectangles",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Radar, Passport, Legend, and Dock remain mutually clear on desktop and narrow viewports",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "mobile, embed, and print keep zero reserve while hidden Legends retain the stage rail",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    const browser = new ChromeVisualBrowser(chromePath);
    try {
      let sessionId = await load(browser, render("architecture", CASES.architecture), {
        width: 720,
        height: 900,
      });
      let receipt = await finalGeometry(browser, sessionId);
