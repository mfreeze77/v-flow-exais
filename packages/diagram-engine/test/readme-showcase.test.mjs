...
function writeStarHistoryCharts(cwd, version) {
  ...
  fs.writeFileSync(
    path.join(assets, "star-history-light.svg"),
    `<svg><title>light ${version}</title></svg>\n`,
  );
  fs.writeFileSync(
    path.join(assets, "star-history-dark.svg"),
    `<svg><title>dark ${version}</title></svg>\n`,
  );
}
...
test("README stays scannable without deleting the visual proof set", () => {
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.match(
      readme,
      filename === "README_ZH.md" ? /不需要绑定代码库/ : /No repository is required/,
    );
...
test("all README languages end with the self-hosted star history chart", () => {
  const lightChart =
    "https://raw.githubusercontent.com/tt-a1i/archify/star-history/assets/star-history-light.svg";
  const darkChart =
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      sectionIndex > contributingIndex,
      `${filename}: Star History must follow Contributing`,
    );
    ...
    assert.equal(
      readme.trimEnd().endsWith("</p>"),
      true,
      `${filename}: Star History must remain the final section`,
    );
  }
  ...
  assert.match(
    workflow,
    /narayann7\/star-history-action@00dfada13f106e4114ee46728aa415857078e76c\s/,
  );
