...
test("landing metadata describes the full technical-diagram product and trusted hero promise", () => {
  ...
  assert.match(
    landing,
    /<meta property="og:title" content="Archify — Technical Diagrams from Plain English">/,
  );
  assert.match(landing, /An agent skill for Cursor, Claude Code, Codex CLI, and OpenCode/);
  assert.equal(
    (
      landing.match(
        /npx -y skills add tt-a1i\/archify --skill archify --agent cursor --global --copy --yes/g,
      ) || []
    ).length,
    2,
  );
...
test("landing hero leads with three real generated proof artifacts", () => {
  ...
  for (const proof of proofs) {
    ...
    assert.ok(
      ...
      `${proof.id}: validation receipt is not green`,
    );
    assert.ok(
      fs.existsSync(path.join(docsRoot, proof.artifact)),
      `${proof.id}: live artifact missing`,
    );
...
test("landing makes Route Journey and core exploration shortcuts discoverable in both languages", () => {
  ...
  assert.match(
    landing,
    /data-i18n="kbd-route">Trace, inspect, and play a route<\/span><kbd>R<\/kbd>/,
  );
