...
test("update awareness is notification-only and never replaces the requested workflow", () => {
  ...
  assert.match(
    skill,
    /`severity` is `security`[\s\S]*security update[\s\S]*emphasis only, never user autonomy/i,
  );
...
test("language behavior stays within the bounded locale contract", () => {
  ...
  assert.match(
    skill,
    /explicit user choice; otherwise follow the request or conversation's dominant language/,
  );
  ...
  assert.match(
    authoringContract,
    /Do not silently substitute\s+`zh-CN` for another language or Chinese locale/,
  );
