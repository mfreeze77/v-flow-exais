...
test("MCO showcase preserves checked-in connection-label geometry", () => {
  ...
  try {
    ...
    assert.ok(
      Object.keys(renderedLabels).length >= 8,
      "expected the authored MCO connection labels",
    );
    assert.deepEqual(
...
test(
  "checked-in MCO artifacts are byte-reproducible from the pinned repository revision",
  {
    skip: pinnedMcoRoot
      ? false
...
test("MCO public proof is source-backed, valid, and linked from every README", () => {
  ...
  assert.match(
    source.components.find((component) => component.id === "router")?.sublabel || "",
    /\bdoctor\b/,
  );
  assert.match(
    source.components.find((component) => component.id === "adapters")?.sublabel || "",
    /\bdetect\b/,
  );
  ...
  assert.ok(
    cardCopy.includes(
      new URL(source.meta.repository.url).host + new URL(source.meta.repository.url).pathname,
    ),
  );
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      readme.includes(`[\`${repositorySlug}\`](${source.meta.repository.url})`),
      `${filename}: repository link drifted`,
    );
