...
test(
  "all packaged HTML examples pass the real visual-check desktop gate",
  {
  ...
  async () => {
    ...
    try {
      ...
      for (const name of packagedHtmlExamples) {
        ...
        assert.equal(
          result.receipt.containment.viewports.every((viewport) => viewport.ok),
          true,
          name,
        );
      }
...
test(
  "production showcase is readable in the real 1440 by 900 adaptive reader",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        assert.equal(
          result.exitCode,
          0,
          `attempt ${attempt}: ${JSON.stringify(result.receipt, null, 2)}`,
        );
        assert.equal(
          result.receipt.readability.status,
          "pass",
          `attempt ${attempt}: ${JSON.stringify(result.receipt, null, 2)}`,
        );
        ...
        const darkDesktop = result.receipt.captures.screenshots.find(
          ({ width, height, theme }) =>
            width === DESKTOP_READABILITY_VIEWPORT.width &&
            height === DESKTOP_READABILITY_VIEWPORT.height &&
            theme === "dark",
