...
test("Semantic Radar tracks desktop camera and mobile contained scroll", () => {
  ...
  assert.match(
    html,
    /nearbyCandidates\(context, reference\)\.concat\(cornerCandidates\(context\)\)/,
  );
  ...
  assert.match(
    html,
    /surface\.addEventListener\('pointerdown',[\s\S]+viewportDrag = \{ pointerId: event\.pointerId \}/,
  );
  assert.match(
    html,
    /target\.closest\('\[data-node-id\], \[data-relationship-hit-key\], \.overview-map'\)/,
  );
...
test(
  "Semantic Radar stays above the measured MAP control strip",
  {
...
test(
  "Semantic Radar avoids an expanded mobile Passport without hiding a collision",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    try {
      ...
      assert.equal(
        overlaps(state.radar, state.passport, 10),
        false,
      ...
      assert.ok(
        expanded.height > state.radar.bottom - state.radar.top,
...
test(
  "Semantic Radar reports a consistent unavailable state and recovers when space returns",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
...
test(
  "Semantic Radar automatically avoids a tall Semantic Passport",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    `,
      ...
      assert.equal(
        overlaps(rects.radar, rects.passport, 10),
        false,
      ...
      await dragMouse(
        ...
        },
        {
          ...
          y:
            dragGeometry.head.top +
            dragGeometry.head.height / 2 +
            dragGeometry.requested.top -
            dragGeometry.radar.top,
        },
      );
      const snappedRects = await radarRects(browser, sessionId, "");
      assert.equal(
        overlaps(snappedRects.radar, snappedRects.passport, 10),
        false,
        JSON.stringify(snappedRects, null, 2),
      );
      assert.ok(
        Math.abs(snappedRects.radar.left - dragGeometry.nearest.left) <= 2,
        JSON.stringify({ dragGeometry, snappedRects }, null, 2),
      );
      assert.ok(
        Math.abs(snappedRects.radar.top - dragGeometry.nearest.top) <= 2,
        JSON.stringify({ dragGeometry, snappedRects }, null, 2),
      );
    } finally {
      await browser.close();
...
test(
  "Semantic Radar titlebar drag persists while surface drag still pans the diagram",
  {
    skip: chromePath ? false : "Set ARCHIFY_CHROME to run the real browser regression.",
  },
  async () => {
    ...
    try {
      ...
      const manuallyPlaced = await evaluate(
        browser,
        sessionId,
        `(function () {
      Archify.radar.sync();
      ...
      assert.ok(
        Math.abs(manuallyPlaced.left - geometry.target.left) <= 2,
        JSON.stringify({ geometry, manuallyPlaced }, null, 2),
      );
      assert.ok(
        Math.abs(manuallyPlaced.top - geometry.target.top) <= 2,
        JSON.stringify({ geometry, manuallyPlaced }, null, 2),
      );
      ...
      const afterSurfaceDrag = await evaluate(
        browser,
        sessionId,
        `(function () {
      var radar = document.getElementById('overview-map').getBoundingClientRect();
      ...
      assert.ok(
        afterResize.radar.left >= Math.max(0, afterResize.container.left),
        JSON.stringify(afterResize, null, 2),
      );
      assert.ok(
        afterResize.radar.right <=
          Math.min(afterResize.viewport.width, afterResize.container.right),
        JSON.stringify(afterResize, null, 2),
      );
      assert.equal(
        overlaps(afterResize.radar, afterResize.controls, 10),
        false,
        JSON.stringify(afterResize, null, 2),
      );
    } finally {
