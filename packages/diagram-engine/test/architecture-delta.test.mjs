...
test("baseline boundary title masks stay below current components and carry delta identity", () => {
  ...
  const rect = (tag) =>
    Object.fromEntries(
      [...tag.matchAll(/\b(x|y|width|height)="([^"]+)"/g)].map((match) => [
        match[1],
        Number(match[2]),
      ]),
    );
  ...
  const overlaps =
    maskBox.x < nodeBox.x + nodeBox.width &&
  ...
  assert.ok(
    delta.indexOf(phantomMask) < currentNode,
    "phantom mask must paint below the current component",
  );
});
...
test("portable compare retains link settings and uses the same repository identity rules", () => {
  ...
  base.meta.repository = {
    url: "https://git.internal/Team/Services/repo.git",
    revision: "a".repeat(40),
    link_mode: "local-only",
  };
  head.meta.repository = {
    url: "https://git.internal:443/Team/Services/repo.git",
  ...
  assert.equal(
    compareArchitecture(base, head, { baseVerified: true, headVerified: true }).proofLevel,
    "revision-pinned",
  );
  head.meta.repository.url = "https://git.internal/team/Services/repo.git";
  assert.throws(
    () => compareArchitecture(base, head),
    (error) => error.code === "delta/repository-mismatch",
  );
  base.meta.repository = {
    url: "https://gitee.com/Team/repo",
    revision: "a".repeat(40),
    provider: "gitee",
  };
