...
test("desktop readability budget matches the minimum adaptive reader at 1440 by 900", () => {
  ...
  assert.match(
    template,
    /@media \(min-width: 768px\) and \(max-height: 1100px\)[\s\S]*?\.diagram-container \{[\s\S]*?padding: 0\.875rem;[\s\S]*?padding-bottom: calc\(0\.875rem \+ var\(--archify-nav-reserve\)\);/,
  );
  assert.match(
    template,
    /@media \(min-width: 768px\) and \(max-height: 920px\)[\s\S]*?body \{ padding-block: 1\.25rem; \}/,
  );
...
test("desktop readability source floor is the inverse of the projected-size gate", () => {
  ...
  assert.equal(
    minimumReadableSourceTextPx(DESKTOP_READER_DIAGRAM_WIDTH),
    MIN_PROJECTED_NODE_TEXT_PX,
  );
