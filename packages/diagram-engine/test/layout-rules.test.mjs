...
const CASES = [
  ...
  [
    "sequence: segment label exceeds segment frame available width",
    "sequence",
    (d) => {
      d.segments[0].label = "Long Segment Label ".repeat(10);
    },
  ...
  [
    "architecture: boundary title must fit inside its own frame",
    "architecture",
    (d) => {
  ...
  [
    "architecture: boundary title must stay inside the authored viewBox",
    "architecture",
    (d) => {
      d.meta.quality_profile = "standard";
      d.meta.viewBox = [500, 400];
...
test("architecture: auto viewBox keeps expanded boundary titles readable at desktop scale", () => {
  ...
  const sourceFontPx = Number(
    html.match(/<text data-boundary-label[^>]*font-size="([\d.]+)"/)?.[1],
  );
...
test("architecture: boundary title masks cannot obscure connection labels", () => {
  const d = {
    ...
    meta: {
      title: "Boundary and connection labels",
...
test("workflow: automatic routing uses one bend and avoids every node border", () => {
  ...
  assert.ok(
    labelY > points[1][1] && labelY < points[0][1],
    "the label should stay inside that segment",
  );
});
...
test("workflow: blocked first one-bend candidate selects the clear one-bend orientation", () => {
  ...
  assert.equal(
    points.length,
    3,
    `expected the alternate one-bend route, received ${JSON.stringify(points)}`,
  );
  assert.equal(
    points[0][1],
    points[1][1],
    "blocked vertical-first candidate must switch to horizontal-first",
  );
...
test("workflow: authored endpoint sides that follow a border are rejected generically", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/endpoint-side-direction\] workflow edges\[0\] id "invalid-drop"/,
  );
...
test("dataflow: authored endpoint sides that follow a border are rejected generically", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/endpoint-side-direction\] dataflow flows\[0\] id "invalid-bottom"/,
  );
  assert.match(stderr, /cross node borders perpendicularly/);
});
...
test("lifecycle: automatic cross-lane routes avoid every state border", () => {
  ...
  assert.equal(code, 0, stderr);
  assertRelationshipsAvoidAllNodeBorders(fs.readFileSync(outPath, "utf8"), d.transitions, d.states);
});
...
test("sequence: segment label exceeding segment frame available width fails layout validation with remediation", () => {
  ...
  d.meta.column_fit = "fixed";
  d.segments[0].label =
    "Phase 1: Dual-Certificate mTLS Handshake & Distributed Token Verification Protocol Negotiation".repeat(
      2,
    );
...
test("sequence: segment label boundary containment within canvas vs segment frame and exact-fit", () => {
  ...
  assert.match(
    resExceed.stderr,
    /label \(~721px\) exceeds the segment frame's available width \(716px\) — shorten the label or increase meta\.viewBox\[0\] to at least 826\./,
  );
  ...
  assert.equal(
    resExact.code,
    0,
    `exact-fit label at segment frame boundary must pass cleanly: ${resExact.stderr}`,
  );
});
