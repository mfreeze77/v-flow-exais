...
if (sourceRef !== "HEAD" && sourceRef !== tag) {
  fail(
    `--source-ref must be HEAD or the exact release tag ${JSON.stringify(tag)}; found ${JSON.stringify(sourceRef)}.`,
  );
}
...
if (version) {
  ...
  try {
    ...
    if (objectType !== "tag") {
      ...
      const epochSource = execFileSync(
        "git",
        ["-C", repoRoot, "for-each-ref", "--format=%(taggerdate:unix)", tagRef],
        {
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"],
        },
...
if (taggerTime && manifest?.publishedAt !== taggerTime) {
  fail(
    `stable update manifest publishedAt ${manifest?.publishedAt || "(missing)"} does not match annotated tagger time ${taggerTime}.`,
  );
}
...
if (treeSha && manifest?.source?.treeSha !== treeSha) {
  fail(
    `stable update manifest treeSha ${manifest?.source?.treeSha || "(missing)"} does not match ${sourceRef}:archify ${treeSha}.`,
  );
}
...
if (archivePath) {
  try {
    archiveSha = crypto
      .createHash("sha256")
      .update(fs.readFileSync(path.resolve(archivePath)))
      .digest("hex");
  } catch {
...
if (archiveSha && manifest?.artifact?.sha256 !== archiveSha) {
  fail(
    `stable update manifest archive sha256 ${manifest?.artifact?.sha256 || "(missing)"} does not match ${archiveSha}.`,
  );
}
