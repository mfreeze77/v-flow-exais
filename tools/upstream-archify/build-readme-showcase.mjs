...
class PipeCdp {
  constructor(child) {
    ...
    child.once("exit", (code) =>
      this.failAll(new Error(`Chrome exited before capture completed (${code})`)),
    );
  }
  ...
  try {
    ...
    for (const [sceneIndex, scene] of scenes.entries()) {
      ...
      await evaluate(
        cdp,
        sessionId,
        `new Promise((resolve,reject)=>{const end=Date.now()+12000;const poll=()=>window.__archifyShowcaseReady?resolve(true):Date.now()>end?reject(new Error('artifact load timeout')):setTimeout(poll,40);poll()})`,
        true,
      );
      ...
      for (let i = 0; i < framesPerScene; i += 1) {
        ...
        await evaluate(
          cdp,
          sessionId,
          `document.documentElement.style.setProperty('--fade','${fade}')`,
        );
...
function buildGif(ffmpeg, framesRoot) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      String(fps),
      "-i",
      path.join(framesRoot, "frame-%04d.png"),
      "-filter_complex",
      filter,
      "-loop",
      "0",
...
async function main() {
  ...
  if (!chromePath)
    throw new Error("Chrome or Chromium is required. Set ARCHIFY_CHROME to its executable path.");
  ...
  try {
    ...
    console.log(
      `README showcase ${frameCount} frames / ${receipt.durationSeconds.toFixed(1)}s / ${receipt.bytes} bytes`,
    );
