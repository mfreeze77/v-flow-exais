...
function run(args, options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error(
      [`archify ${args.join(" ")} failed with ${result.status}`, result.stdout, result.stderr]
        .filter(Boolean)
        .join("\n"),
    );
  }
...
try {
  ...
  const repositoryNotices = fs.readFileSync(
    path.join(noticeComparisonRoot, "THIRD_PARTY_NOTICES.md"),
    "utf8",
  );
  ...
  if (declaredDependencyField) {
    throw new Error(
      `packaged skill must not declare dependency metadata: ${declaredDependencyField}`,
    );
  }
  ...
  if (updateCheck.status !== 0) {
    throw new Error(
      `packaged update checker failed with ${updateCheck.status}\n${updateCheck.stderr}`,
    );
  }
  let updateReceipt;
  try {
  ...
  if (!versionCore)
    throw new Error("package version cannot produce an update-check smoke candidate");
  ...
  if (notifierReceipt.status !== "update_available") {
    throw new Error(
      `packaged update checker did not return an update candidate: ${JSON.stringify(notifierReceipt)}`,
    );
  }
  ...
  if (capturedPreset.brand !== "github" || capturedPreset.evidence.status !== "preset") {
    throw new Error(
      "packaged brand capture did not resolve a known domain without network capture",
    );
  }
  ...
  const workflowLayout = JSON.parse(
    run([
      "validate",
      "workflow",
  ...
  if (
    workflowLayout.contract !== "readable-v2" ||
    workflowLayout.columns?.length !== 6 ||
    workflowLayout.diagnostics?.length !== 0
  ) {
    throw new Error(
      "packaged workflow compiler did not expose a passing readable-v2 layout receipt",
    );
  }
  ...
  if (
    !migrationReceipt.ok ||
    migrationReceipt.fromSchemaVersion !== 1 ||
    migrationReceipt.toSchemaVersion !== 2 ||
    !fs.existsSync(migratedWorkflowPath)
  ) {
    throw new Error("packaged workflow migrator did not produce a schema-v2 destination");
  }
  const idempotenceReceipt = JSON.parse(
    run([
      "migrate",
      "workflow",
      migratedWorkflowPath,
  ...
  if (
    !idempotenceReceipt.ok ||
    idempotenceReceipt.fromSchemaVersion !== 2 ||
  ...
  if (
    !compareReceipt.ok ||
    compareReceipt.completeness !== "complete" ||
  ...
  if (
    !delivered.ok ||
    delivered.validation?.compositionStatus !== "pass" ||
  ...
  const visualSkipped = JSON.parse(
    runExpectFailure(["visual-check", delivered.output, "--json"], {
      env: { ...process.env, ARCHIFY_CHROME: path.join(scratch, "missing-chrome") },
    }),
  );
  if (
    visualSkipped.status !== "skipped" ||
    visualSkipped.visualReview !== "pending" ||
  ...
  if (
    failure.ok ||
    !failure.diagnostics?.some((diagnostic) => diagnostic.code === "schema/additionalProperties")
  ) {
  ...
  const tangentFailure = JSON.parse(
    runExpectFailure(["validate", "architecture", tangentPath, "--json"]),
  );
  const tangentDiagnostic = tangentFailure.diagnostics?.find(
    (diagnostic) => diagnostic.code === "clean-flow/endpoint-side-direction",
  );
  if (
    tangentFailure.ok ||
    tangentDiagnostic?.evidence?.authoredField !== "toSide" ||
    tangentDiagnostic?.evidence?.side !== "top"
  ) {
    throw new Error(
      "packaged skill did not reject a tangential target-port entry with structured repair evidence",
    );
  }
  ...
  if (
    !inferredBridgeArtifact.includes('data-composition-points="700,130;184,130;184,330;160,330"')
  ) {
    throw new Error(
      "packaged skill did not preserve inferred endpoint normals with a side-aware bridge",
    );
  }
