...
const EXCLUDED_SEGMENTS = new Set([".DS_Store", ".hive", ".workbuddy", "node_modules"]);
...
function excluded(relative) {
  ...
  return insideSkill
    .split("/")
    .some((segment) => EXCLUDED_SEGMENTS.has(segment) || segment.startsWith(".validator-check-"));
}
...
function preflightSourceEntry(repoRoot, entry) {
  ...
  for (const [index, segment] of segments.entries()) {
    ...
    if (metadata.isSymbolicLink()) {
      ...
      throw new Error(
        `refusing to package path through symlink: ${traversed} (for ${entry.relative})`,
      );
    }
...
function snapshotSourceEntry(entry) {
  ...
  try {
    ...
    if (!after.isFile() || !sameSnapshotState(before, after) || !sourcePathUnchanged(entry)) {
...
function validateThirdPartyNoticeInputs(repoRoot, packageEntries) {
  const packagedEntry = packageEntries.find(
    (entry) => entry.relative === "archify/THIRD_PARTY_NOTICES.md",
  );
  if (!packagedEntry) {
  ...
  if (
    embeddedFonts &&
    !packageEntries.some((entry) => entry.relative === "archify/assets/JetBrainsMono-OFL.txt")
  ) {
...
export function stageCleanSkill({ repoRoot = scriptRoot, destination }) {
  ...
  for (const entry of entries) {
    if (entry.stage !== "0") {
      throw new Error(
        `refusing to package unmerged index entry (stage ${entry.stage}): ${entry.relative}`,
      );
    }
  }
