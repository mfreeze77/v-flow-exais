...
function checkSkillRelease(value, version, isDevelopment) {
  ...
  fail(
    `archify/skill-release.json must identify archify ${version} as ${expectedChannel}, use the official repository, and pin the trusted update manifest URL.`,
  );
}

function checkStableUpdateManifest(
  value,
  expectedVersion,
  previousVersion,
  packageVersion,
  ...
  try {
    ...
    if (
      expectedVersion &&
      allowedVersions.includes(manifest.version) &&
      (isDevelopment || packageVersion === expectedVersion)
    )
      return;
  } catch {
    // Report the repository-specific cross-artifact requirement below.
  ...
  fail(
    `docs/skill-updates/archify/stable.json must describe the ${releaseWindow} with the fixed repository, immutable tree/archive digests, and official release-notes URL.`,
  );
}
...
function versionLabels(source) {
  return [...source.matchAll(/\bv(\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?)\b/g)].map(
    (match) => match[1],
  );
}
...
function checkReadme(relativePath, source, version, language, isDevelopment) {
  ...
  const hasMarker = source
    .split("\n")
    .some((line) => line.includes(markerLabel) && line.includes(`\`v${version}\``));
  if (!source.includes(badge) || !hasMarker) {
    fail(
      `${relativePath} must advertise ${identity} identity v${version} with an exact escaped badge and explicit ${identity} marker.`,
    );
  }
}
...
function checkDocument(relativePath, source, version, isDevelopment) {
  ...
  if (
    labels.length === 0 ||
    labels.some((label) => label !== version) ||
    !englishLabel.test(source) ||
    !chineseLabel.test(source)
  ) {
    fail(
      `${relativePath} must advertise ${identity} identity v${version} in both languages without a conflicting version alias.`,
    );
  }
}
...
function checkRavenBoundary(relativePath, source, language) {
  ...
  const hasCorrectDestination =
    language === "both"
      ? englishExtractsIntoParent &&
        englishExplainsInstalledRoot &&
        chineseExtractsIntoParent &&
        chineseExplainsInstalledRoot
      : language === "zh"
  ...
  const inventsSwitcher =
    /data-agent=["']raven["']/i.test(source) ||
  ...
  if (
    ...
    fail(
      `${relativePath}: Raven must remain a manual ZIP installation outside the agent switcher: extract archify.zip into ~/.raven/workspace/skills, yielding ~/.raven/workspace/skills/archify.`,
    );
  }
}

function checkIdentityTemplate(relativePath, source, isDevelopment) {
  const hasHardcodedVersion = /\b\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?\b/.test(
    source,
  );
  ...
  if (!source.includes("[[ARCHIFY_VERSION]]") || !hasIdentity || hasHardcodedVersion) {
    fail(
      `${relativePath} must use [[ARCHIFY_VERSION]] with ${identity} labels, never a hardcoded package version.`,
    );
  }
...
const hasSupportedVersion = Boolean(
  parsedVersion && parsedVersion.build === null && supportedPrerelease,
);
...
for (const label of publishedLabels) {
  if (!isStableCoreVersion(label))
    fail(`CHANGELOG published version is not stable SemVer: ${label}.`);
}
...
if (hasSupportedVersion && hasRealUnreleasedChanges) {
  ...
  if (newestStableLabel && compareSemver(currentCore, newestStableLabel) <= 0) {
    fail(
      `Unreleased package core ${currentCore} must be newer than published ${newestStableLabel}.`,
    );
  }
}
...
if (hasSupportedVersion) {
  ...
  if (skillVersion !== expectedSkillVersion) {
    fail(
      `archify/SKILL.md metadata version ${skillVersion || "(missing)"} must map to package ${version} as ${expectedSkillVersion}.`,
    );
  }

  const rendererTemplate = read("archify/assets/template.html");
  const generatorVersions = [
    ...rendererTemplate.matchAll(/<meta\s+name="generator"\s+content="archify\s+([^"]+)"\s*\/?>/g),
  ].map((match) => match[1]);
  if (generatorVersions.length !== 1 || generatorVersions[0] !== version) {
    fail(
      `archify/assets/template.html generator must be archify ${version}; found ${generatorVersions.join(", ") || "(missing)"}.`,
    );
  }
  ...
  if (newestStableLabel && isDevelopment) {
    ...
    if (
      new RegExp(`Archify ${stableMinor} includes\\b`).test(english) ||
      new RegExp(`Archify ${stableMinor} 已覆盖`).test(chinese)
    ) {
      fail(
        `README capability summary must describe v${version} as development, not published ${newestStableLabel}.`,
      );
    }
  }
  ...
  if (proofCounts.length === 0 || staleProofCounts.length > 0) {
    ...
    fail(
      `docs/index.html proof receipt must say 9/9 everywhere; every N/N proof receipt must be exactly 9/9${found}.`,
    );
  }
