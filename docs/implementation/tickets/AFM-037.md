...
**Status:** IN_PROGRESS — local source/diagram comparison checkpoint verified; prerequisite and product integration acceptance remain open

## Checkpoint — 2026-09-11

Implemented immutable local before/head Git capture, exact tree-entry deltas,
validated architecture comparison through the owned library, source citation
checks and persisted receipts. Sixteen focused tests and 42 inherited
delta/evidence tests pass, including pinned-browser evidence. The real repository
comparison records 38 changed paths. See
[`result.json`](../../../evidence/tickets/AFM-037/result.json) and
[`revision-review.md`](../../designs/revision-review.md).

AFM-018/024/029 completion, full Studio review integration and downstream
editable change films remain open. This is not a full-product gate pass.
...
- [x] Delta output corresponds to the exact two supplied source revisions (local Git and separately authored diagram inputs).
- [x] Unreadable or stale evidence is distinguishable from verified evidence (captured path/hash/line checks and revision conflicts).
- [x] Repeated/parallel relationships cannot be matched only by endpoints (authored relationship IDs are required).
