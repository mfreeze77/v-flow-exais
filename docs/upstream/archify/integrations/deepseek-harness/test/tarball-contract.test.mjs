...
test("pack command emits a real npm tarball with the expected identity and file list", () => {
  ...
  try {
    ...
    const skillEntries = files.filter(
      (file) => file === "skills/archify/SKILL.md" || file.endsWith("/SKILL.md"),
    );
...
test("packed Skill payload remains byte-identical to the declared immutable source commit", () => {
  ...
  try {
    ...
    assert.deepEqual(
      JSON.parse(fs.readFileSync(path.join(packedRoot, "package", "release.json"), "utf8")),
      release,
    );
    ...
    for (const packagedPath of skillFiles) {
      ...
      const tagged = spawnSync("git", ["show", `${DSH_RELEASE_REF}:archify/${relative}`], {
        cwd: repoRoot,
        encoding: null,
      });
