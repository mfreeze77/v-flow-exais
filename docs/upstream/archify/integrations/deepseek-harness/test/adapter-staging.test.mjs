...
test("pack rejects a committed reserved device-name path and leaves no target tarball", () => {
  ...
  try {
    ...
    const blob = git(checkout, ["hash-object", "-w", "--stdin"], {
      input: "reserved-name fixture\n",
    });
...
test("pack rejects committed adapter paths that collide by case and leaves no target tarball", () => {
  ...
  try {
    ...
    assert.match(
      git(checkout, ["ls-tree", "HEAD", "--", "integrations/deepseek-harness/lib/index.js"]),
      /^100644 blob /,
    );
    ...
    assert.match(
      `${result.stderr}\n${result.stdout}`,
      /collide across supported filesystems|INDEX\.js|index\.js/i,
    );
    assert.equal(fs.existsSync(out), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
