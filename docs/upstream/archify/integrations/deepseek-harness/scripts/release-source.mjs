...
function readGit(args) {
  const result = spawnCliSync("git", args, {
    cwd: repoRoot,
    encoding: null,
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      `unable to read committed DSH adapter source: ${result.stderr?.toString("utf8") || result.error?.message}`,
    );
  }
...
const adapterEntries = new TextDecoder("utf-8", { fatal: true })
  ...
  .map((record) => {
    ...
    return {
      ...
      type,
      objectId,
      relative: record.slice(separator + 1).slice(adapterPrefix.length),
    };
  })
  ...
  adapterEntries.map((entry) => {
    ...
    for (const [index, segment] of segments.entries()) {
      ...
      if (portablePaths.has(key) && portablePaths.get(key) !== prefix) {
        throw new Error(
          `DSH adapter paths collide across supported filesystems: ${portablePaths.get(key)} and ${prefix}`,
        );
      }
    ...
    if (entry.type !== "blob" || !["100644", "100755"].includes(entry.mode)) {
      throw new Error(
        `DSH adapter input must be a regular Git file: ${entry.relative} (${entry.mode})`,
      );
    }
...
export function releaseSnapshot(destination) {
  ...
  for (const args of [
    ...
    if (result.error || result.status !== 0) {
      throw new Error(
        `unable to read DSH source ${release.sourceCommit}: ${result.stderr || result.error?.message}`,
      );
    }
  }
  const skill = JSON.parse(
    fs.readFileSync(path.join(destination, "archify", "package.json"), "utf8"),
  );
