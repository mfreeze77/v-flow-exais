...
try {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      packMeta =
        Object.values(parsed || {}).find((entry) => entry?.name === "@tt-a1i/archify-dsh") || {};
    }
  } catch {
    packMeta = {};
  }
  ...
  const packagedPaths = new Set(
    files.map(({ path: filePath }) => filePath.replace(/^package\//, "")),
  );
  for (const required of [
    ...
    if (!packagedPaths.has(required))
      throw new Error(`DSH tarball is missing required file: ${required}`);
  }
