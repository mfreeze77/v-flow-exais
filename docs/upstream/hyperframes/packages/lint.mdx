...
<Tip>
  This package is the payoff of running validation **as a library** instead of shelling out to the
  CLI. A Node app (an agent harness, a CI step, an editor plugin) can `import {lintProject} from
  '@hyperframes/lint'` and lint a composition directory directly — no `npx hyperframes lint`
  subprocess, no stdout parsing.
</Tip>
...
<Info>
  `@hyperframes/core/lint` still resolves (via a back-compat re-export stub), so existing imports
  keep working. New code should import from `@hyperframes/lint` directly.
</Info>
...
<Info>
  `lintProject` (which walks a project **directory**) is filesystem-based and is **not** part of the
  browser entry — import it from the main `@hyperframes/lint` entry in Node.
</Info>
...
<Info>For common failures and fixes, see [Troubleshooting](/guides/troubleshooting).</Info>
