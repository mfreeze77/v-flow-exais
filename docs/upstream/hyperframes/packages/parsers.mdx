...
<Tip>
  **Most users do not need to install `@hyperframes/parsers` directly.**
  [`@hyperframes/core`](/packages/core) re-exports the parser API it needs, and the
  [CLI](/packages/cli) / [studio](/packages/studio) depend on it transitively. Reach for it directly
  only when you want the parsing layer **without** pulling in the rest of core.
</Tip>
...
<Info>
  The package ships focused subpath entries. Import `@hyperframes/parsers/hf-ids` when you do not
  need the GSAP AST machinery.
</Info>
...
```typescript
import { updateElementInHtml, addElementToHtml, removeElementFromHtml } from "@hyperframes/parsers";
