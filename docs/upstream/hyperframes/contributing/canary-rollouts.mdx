...
<Note>
  A canary is a *measured* rollout: a slice is enrolled so it can be compared against everyone else.
  So **opting out of telemetry opts you out of canaries** — an opted-out install is never bucketed
  at all. See the [`telemetry` command](/packages/cli#telemetry) for the opt-out routes.
</Note>
...
**4. Delete it** at 100 and holding — the registry entry _and_ the branch it
...
or a CI machine all resolve to _not enrolled_. At exactly 100% those exclusions
