...
<Note>
  This API lives on the `@hyperframes/sdk/editing` subpath, available since
  `@hyperframes/sdk@0.7.22`. If the import fails to resolve, upgrade: `npm install
  @hyperframes/sdk@latest`.
</Note>
...
<Warning>
  `canMove` and `canResize` indicate that the element has authored `left`/`top`/`width`/`height`
  style values that can be directly edited as fields. A canvas drag operation maps to
  `canApplyManualOffset`, not `canMove`. Do not gate drag handles on `canMove`.
</Warning>
...
function onElementSelected(hfId: string) {
  ...
  const liveEl = iframe.contentDocument?.querySelector<HTMLElement>(`[data-hf-id="${hfId}"]`);
  if (!liveEl) return;
