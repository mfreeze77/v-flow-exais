...
{/_ VISUAL NEEDED (2): color wheels, a bent RGB curve, a hue curve and one HSL
selection, each beside the frame it produced. _/}
...
- **Waveform** — brightness across the frame left to right, so you see _which
  part_ is blown out.
...
{/_ VISUAL NEEDED (3): the four scopes side by side, on the same frame before
and after a correction. _/}
...
{/_ VISUAL NEEDED (4): a real grade-compare.png sheet — the labelled grid with
`original` in the first cell. _/}
...
{/_ VISUAL NEEDED (5): one frame under two or three .cube LUTs, plus the same
LUT at partial and full intensity. _/}
...
<video
  src="./interview.mp4"
  data-color-grading='{"preset":"skin-soft","intensity":0.7,"adjust":{"exposure":0.15},"effects":{"bloom":0.4}}'
></video>
```
