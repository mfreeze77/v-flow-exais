...
<Tip>
  No account is needed to try HyperFrames locally. With no credential, voice can use **Kokoro** and
  music can use **MusicGen** — see [Working offline](#working-offline).
</Tip>
...
<Note>
  `npx hyperframes tts` itself is the local Kokoro CLI. Hosted HeyGen and ElevenLabs voices are
  selected by the bundled media workflow helpers, not by that command.
</Note>
