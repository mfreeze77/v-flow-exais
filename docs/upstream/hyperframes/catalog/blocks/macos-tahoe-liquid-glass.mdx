...
<video
  className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800"
...
<InstallCommand
  command="npx hyperframes add macos-tahoe-liquid-glass"
  item="macos-tahoe-liquid-glass"
/>
...
<Danger>
  Live preview needs the `chrome://flags/#canvas-draw-element` flag switched on. Without it this
  item's screen renders black. Rendering from the CLI switches it on for you. [How it
