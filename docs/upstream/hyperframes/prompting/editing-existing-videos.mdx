...
<DocsVideo
  ...
  loop
/>
*After — four verbatim edits from this page (move, right-trim, retime, restyle-with-freeze); each
touched exactly the attribute the verb map promises, gates green after every edit.*
...
## Make it snappier (retiming _feel_, not just duration)
...
<Tip>
  For an element-specific edit, the Design Panel's clipboard icon copies structured context — the
  element's id, position, size, and current animation — ready to paste into your prompt. It gives
  the agent exact spatial context instead of a vague "the title."
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Studio timeline" href="/studio/timeline">
    Move, trim, split, and navigate clips
  </Card>
  <Card title="Data attributes" href="/concepts/data-attributes">
    The `data-*` timing contract
  </Card>
</CardGroup>

_Next: [Iterating](/prompting/iterating) — the discipline behind every edit on this page: one variable per render, absolute targets, freeze what works._
