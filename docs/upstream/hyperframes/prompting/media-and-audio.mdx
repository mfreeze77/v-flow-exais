...
HyperFrames owns media _playback_. A sibling media pipeline resolves everything
...
Music resolves from a large catalog by mood. It should almost always sit _under_
...
<Tip>
  A stated loudness target ("under −18 dB," "ducked under the voice") is the difference between
  music that supports the piece and music that buries it. When there's narration, always say the bed
  goes under it.
</Tip>
...
Caption _look_ is its own vocabulary — tone, size, per-word emphasis. See the
...
> Layer the headline _behind_ the presenter so their silhouette occludes the text.
...
- A cutout that reveals into view goes inside a non-timed `<div>`, and the
  _wrapper_ is what gets animated. HyperFrames owns clip visibility, so
...
<DocsVideo
  ...
  loop
/>
*Rendered from the prompt above, unedited. The source clip is a plain H.265/HEVC file. The render
decoded it directly via FFmpeg. Preview would have used the automatic H.264 proxy.*
...
agent will search when you only describe an asset. A path settles _which_ file.

- ❌ `use my logo`
- ✅ `use assets/logo.svg`

This matters even when a search would have worked. Brand and entity assets
should point at _your_ file, not a resolved lookalike.
...
<Note>
  **Capstone thread** — the [Level 7 film](/prompting/capstone)'s Material region runs this
  chapter's entire pipeline on one clip: generated footage → HEVC auto-proxy → background removal
  mid-scene → word-synced captions from the clip's own transcription, with the clip's audio ducking
  the BGM (cut from the film, below).
</Note>
...
_Next: [Audio effects and mixing](/prompting/audio-effects) — making the voice,
music, and effects you just placed sit together._
