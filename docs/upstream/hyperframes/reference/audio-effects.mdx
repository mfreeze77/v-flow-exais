...
| `curve`         | −1 to 1, bends the segment _leaving_ a point; positive holds low then rises late                                           |
...
A carve is a **relationship, not an effect**. The settings live on the _bed_ —
...
member classified voice or unknown that this run did not analyse is _not_ a
