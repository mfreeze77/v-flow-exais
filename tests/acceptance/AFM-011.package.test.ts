...
function render(kind: string, example: string, output: string) {
  isolatedNode([join(installedEngine, "renderers", kind, `render-${kind}.mjs`), example, output]);
...
describe("AFM-011: actual installed archives without source-checkout access", () => {
  it("denies reads of the owned checkout and frozen raw inputs", () => {
    for (const path of [
      ...
      join(root, "_sources/archify-main/archify/package.json"),
    ]) {
      expect(() =>
        isolatedNode(["-e", `require('node:fs').readFileSync(${JSON.stringify(path)})`]),
      ).toThrow(/ERR_ACCESS_DENIED/);
    }
  });
  ...
  it("renders all five families using only installed package contents", () => {
    ...
    for (const [kind, example] of Object.entries(examples)) {
      const html = render(
        kind,
        join(installedEngine, "examples", example),
        join(consumer, `${kind}.html`),
      );
  ...
  it("preserves explicit relationship identities, endpoints and arrow direction", () => {
    const source = JSON.parse(
      readFileSync(
        join(installedEngine, "examples/production-deployment.architecture.json"),
        "utf8",
      ),
    );
    source.connections.forEach((edge: { id?: string }, index: number) => {
      edge.id = `deployment-edge-${index + 1}`;
    });
    ...
    expect(
      ...
      source.connections.map(({ id, from, to }: { id: string; from: string; to: string }) => ({
        id,
        from,
        to,
      })),
  ...
  it("fails when the packed viewer template is missing instead of reaching into the checkout", () => {
    ...
    try {
      expect(() =>
        render(
          "architecture",
          join(installedEngine, "examples/web-app.architecture.json"),
          join(consumer, "missing-template.html"),
        ),
